@testset "MonteCarlo" begin
    ...
    for i = 1:N
        ...
        @test MonteCarlo.shiftT!(oldT, newT, step, β) * MonteCarlo.shiftT!(newT, oldT, step, β) ≈ 1.0
        @test oldT >= 0.0 && oldT < β
        @test newT >= 0.0 && newT < β
        @test MonteCarlo.shiftT_flip!(oldT, newT, β) * MonteCarlo.shiftT_flip!(newT, oldT, β) ≈ 1.0
        @test oldT >= 0.0 && oldT < β
        @test newT >= 0.0 && newT < β
    end
    ...
    newK2=oldK2*2.0
    for i = 1:N
        @test MonteCarlo.createFermiK!(newK2, Kf, δK)*MonteCarlo.removeFermiK(newK2, Kf, δK) ≈ 1.0
        @test MonteCarlo.createFermiK!(newK3, Kf, δK)*MonteCarlo.removeFermiK(newK3, Kf, δK) ≈ 1.0
    end

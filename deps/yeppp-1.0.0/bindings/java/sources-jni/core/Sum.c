...
JNIEXPORT jfloat JNICALL Java_info_yeppp_Core_Sum_1V32f_1S32f(JNIEnv *env, jclass class, jfloatArray vArray, jint vOffset, jint length) {
	...
	status = yepCore_Sum_V32f_S32f(&vPointer[vOffset], &sum, length);
...
JNIEXPORT jdouble JNICALL Java_info_yeppp_Core_Sum_1V64f_1S64f(JNIEnv *env, jclass class, jdoubleArray vArray, jint vOffset, jint length) {
	enum YepStatus status;
	...
	status = yepCore_Sum_V64f_S64f(&vPointer[vOffset], &sum, length);

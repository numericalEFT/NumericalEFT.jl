;                       Yeppp! library implementation
;                   This file is auto-generated by Peach-Py,
;        Portable Efficient Assembly Code-generator in Higher-level Python,
;                  part of the Yeppp! library infrastructure
; This file is part of Yeppp! library and licensed under the New BSD license.
; See LICENSE.txt for the full text of the license.

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8sV8s_V8s_Nehalem
_yepCore_Subtract_V8sV8s_V8s_Nehalem:
%else
section .text
global __yepCore_Subtract_V8sV8s_V8s_Nehalem
__yepCore_Subtract_V8sV8s_V8s_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], al
	ADD rdx, 1
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 128
	JB .batch_process_finish
	.process_batch_prologue:
	MOVDQU xmm0, [rdi]
	MOVDQU xmm8, [byte rdi + 16]
	MOVDQU xmm1, [rsi]
	MOVDQU xmm9, [byte rdi + 32]
	MOVDQU xmm13, [byte rsi + 16]
	MOVDQU xmm12, [byte rdi + 48]
	MOVDQU xmm10, [byte rsi + 32]
	MOVDQU xmm14, [byte rdi + 64]
	MOVDQU xmm3, [byte rsi + 48]
	MOVDQU xmm11, [byte rdi + 80]
	MOVDQU xmm6, [byte rsi + 64]
	MOVDQU xmm4, [byte rdi + 96]
	MOVDQU xmm7, [byte rsi + 80]
	PSUBB xmm0, xmm1
	MOVDQU xmm2, [byte rdi + 112]
	MOVDQU xmm5, [byte rsi + 96]
	PSUBB xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 128
	MOVDQU xmm15, [byte rsi + 112]
	PSUBB xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 128
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVDQU xmm0, [rdi]
	ADD rsi, 128
	PSUBB xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	MOVDQU xmm8, [byte rdi + 16]
	MOVDQU xmm1, [rsi]
	PSUBB xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	MOVDQU xmm9, [byte rdi + 32]
	MOVDQU xmm13, [byte rsi + 16]
	PSUBB xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	MOVDQU xmm12, [byte rdi + 48]
	MOVDQU xmm10, [byte rsi + 32]
	PSUBB xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	MOVDQU xmm14, [byte rdi + 64]
	MOVDQU xmm3, [byte rsi + 48]
	PSUBB xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQU xmm11, [byte rdi + 80]
	MOVDQU xmm6, [byte rsi + 64]
	MOVDQA [byte rdx + 112], xmm2
	MOVDQU xmm4, [byte rdi + 96]
	MOVDQU xmm7, [byte rsi + 80]
	PSUBB xmm0, xmm1
	ADD rdx, 128
	MOVDQU xmm2, [byte rdi + 112]
	MOVDQU xmm5, [byte rsi + 96]
	PSUBB xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 128
	MOVDQU xmm15, [byte rsi + 112]
	PSUBB xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 128
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	PSUBB xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBB xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBB xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBB xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBB xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 128
	JZ .return_ok
	.process_single:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], al
	ADD rdx, 1
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8sV8s_V8s_SandyBridge
_yepCore_Subtract_V8sV8s_V8s_SandyBridge:
%else
section .text
global __yepCore_Subtract_V8sV8s_V8s_SandyBridge
__yepCore_Subtract_V8sV8s_V8s_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], al
	ADD rdx, 1
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 128
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVDQU xmm0, [rdi]
	VMOVDQU xmm8, [byte rdi + 16]
	VMOVDQU xmm1, [rsi]
	VMOVDQU xmm9, [byte rdi + 32]
	VMOVDQU xmm13, [byte rsi + 16]
	VMOVDQU xmm12, [byte rdi + 48]
	VMOVDQU xmm10, [byte rsi + 32]
	VMOVDQU xmm14, [byte rdi + 64]
	VMOVDQU xmm3, [byte rsi + 48]
	VMOVDQU xmm11, [byte rdi + 80]
	VMOVDQU xmm6, [byte rsi + 64]
	VMOVDQU xmm4, [byte rdi + 96]
	VMOVDQU xmm7, [byte rsi + 80]
	VPSUBB xmm0, xmm0, xmm1
	VMOVDQU xmm2, [byte rdi + 112]
	VMOVDQU xmm5, [byte rsi + 96]
	VPSUBB xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 128
	VMOVDQU xmm15, [byte rsi + 112]
	VPSUBB xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 128
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVDQU xmm0, [rdi]
	ADD rsi, 128
	VPSUBB xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VMOVDQU xmm8, [byte rdi + 16]
	VMOVDQU xmm1, [rsi]
	VPSUBB xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VMOVDQU xmm9, [byte rdi + 32]
	VMOVDQU xmm13, [byte rsi + 16]
	VPSUBB xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VMOVDQU xmm12, [byte rdi + 48]
	VMOVDQU xmm10, [byte rsi + 32]
	VPSUBB xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VMOVDQU xmm14, [byte rdi + 64]
	VMOVDQU xmm3, [byte rsi + 48]
	VPSUBB xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQU xmm11, [byte rdi + 80]
	VMOVDQU xmm6, [byte rsi + 64]
	VMOVDQA [byte rdx + 112], xmm2
	VMOVDQU xmm4, [byte rdi + 96]
	VMOVDQU xmm7, [byte rsi + 80]
	VPSUBB xmm0, xmm0, xmm1
	ADD rdx, 128
	VMOVDQU xmm2, [byte rdi + 112]
	VMOVDQU xmm5, [byte rsi + 96]
	VPSUBB xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 128
	VMOVDQU xmm15, [byte rsi + 112]
	VPSUBB xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 128
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBB xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBB xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBB xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBB xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBB xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 128
	JZ .return_ok
	.process_single:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], al
	ADD rdx, 1
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8sV8s_V8s_Haswell
_yepCore_Subtract_V8sV8s_V8s_Haswell:
%else
section .text
global __yepCore_Subtract_V8sV8s_V8s_Haswell
__yepCore_Subtract_V8sV8s_V8s_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], al
	ADD rdx, 1
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 256
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVDQU ymm0, [rdi]
	VMOVDQU ymm8, [byte rdi + 32]
	VMOVDQU ymm7, [rsi]
	VMOVDQU ymm9, [byte rdi + 64]
	VMOVDQU ymm5, [byte rsi + 32]
	VMOVDQU ymm1, [byte rdi + 96]
	VMOVDQU ymm3, [byte rsi + 64]
	VMOVDQU ymm14, [dword rdi + 128]
	VMOVDQU ymm2, [byte rsi + 96]
	VMOVDQU ymm12, [dword rdi + 160]
	VMOVDQU ymm15, [dword rsi + 128]
	VMOVDQU ymm11, [dword rdi + 192]
	VMOVDQU ymm10, [dword rsi + 160]
	VPSUBB ymm0, ymm0, ymm7
	VMOVDQU ymm13, [dword rdi + 224]
	VMOVDQU ymm4, [dword rsi + 192]
	VPSUBB ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 256
	VMOVDQU ymm6, [dword rsi + 224]
	VPSUBB ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 256
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVDQU ymm0, [rdi]
	ADD rsi, 256
	VPSUBB ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VMOVDQU ymm8, [byte rdi + 32]
	VMOVDQU ymm7, [rsi]
	VPSUBB ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VMOVDQU ymm9, [byte rdi + 64]
	VMOVDQU ymm5, [byte rsi + 32]
	VPSUBB ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VMOVDQU ymm1, [byte rdi + 96]
	VMOVDQU ymm3, [byte rsi + 64]
	VPSUBB ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VMOVDQU ymm14, [dword rdi + 128]
	VMOVDQU ymm2, [byte rsi + 96]
	VPSUBB ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQU ymm12, [dword rdi + 160]
	VMOVDQU ymm15, [dword rsi + 128]
	VMOVDQA [dword rdx + 224], ymm13
	VMOVDQU ymm11, [dword rdi + 192]
	VMOVDQU ymm10, [dword rsi + 160]
	VPSUBB ymm0, ymm0, ymm7
	ADD rdx, 256
	VMOVDQU ymm13, [dword rdi + 224]
	VMOVDQU ymm4, [dword rsi + 192]
	VPSUBB ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 256
	VMOVDQU ymm6, [dword rsi + 224]
	VPSUBB ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 256
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 256
	VPSUBB ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBB ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBB ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBB ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBB ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 256
	JZ .return_ok
	.process_single:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], al
	ADD rdx, 1
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.K10 progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8sV8s_V16s_K10
_yepCore_Subtract_V8sV8s_V16s_K10:
%else
section .text
global __yepCore_Subtract_V8sV8s_V16s_K10
__yepCore_Subtract_V8sV8s_V16s_K10:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	MOVQ xmm3, [rdi]
	MOVQ xmm2, [rsi]
	PXOR xmm1, xmm1
	PXOR xmm0, xmm0
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PXOR xmm7, xmm7
	PXOR xmm11, xmm11
	PCMPGTB xmm1, xmm3
	PCMPGTB xmm0, xmm2
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PXOR xmm12, xmm12
	PXOR xmm6, xmm6
	PCMPGTB xmm7, xmm8
	PCMPGTB xmm11, xmm14
	PUNPCKLBW xmm3, xmm1
	PUNPCKLBW xmm2, xmm0
	PSUBW xmm3, xmm2
	MOVQ xmm13, [byte rdi + 24]
	MOVQ xmm5, [byte rsi + 24]
	PXOR xmm4, xmm4
	PXOR xmm15, xmm15
	PCMPGTB xmm12, xmm9
	PCMPGTB xmm6, xmm10
	PUNPCKLBW xmm8, xmm7
	PUNPCKLBW xmm14, xmm11
	PSUBW xmm8, xmm14
	MOVDQA [rdx], xmm3
	ADD rdi, 32
	ADD rsi, 32
	PCMPGTB xmm4, xmm13
	PCMPGTB xmm15, xmm5
	PUNPCKLBW xmm9, xmm12
	PUNPCKLBW xmm10, xmm6
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVQ xmm3, [rdi]
	MOVQ xmm2, [rsi]
	PXOR xmm1, xmm1
	PXOR xmm0, xmm0
	PUNPCKLBW xmm13, xmm4
	PUNPCKLBW xmm5, xmm15
	PSUBW xmm13, xmm5
	MOVDQA [byte rdx + 32], xmm9
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PXOR xmm7, xmm7
	PXOR xmm11, xmm11
	PCMPGTB xmm1, xmm3
	PCMPGTB xmm0, xmm2
	MOVDQA [byte rdx + 48], xmm13
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PXOR xmm12, xmm12
	PXOR xmm6, xmm6
	PCMPGTB xmm7, xmm8
	PCMPGTB xmm11, xmm14
	PUNPCKLBW xmm3, xmm1
	PUNPCKLBW xmm2, xmm0
	PSUBW xmm3, xmm2
	ADD rdx, 64
	MOVQ xmm13, [byte rdi + 24]
	MOVQ xmm5, [byte rsi + 24]
	PXOR xmm4, xmm4
	PXOR xmm15, xmm15
	PCMPGTB xmm12, xmm9
	PCMPGTB xmm6, xmm10
	PUNPCKLBW xmm8, xmm7
	PUNPCKLBW xmm14, xmm11
	PSUBW xmm8, xmm14
	MOVDQA [rdx], xmm3
	ADD rdi, 32
	ADD rsi, 32
	PCMPGTB xmm4, xmm13
	PCMPGTB xmm15, xmm5
	PUNPCKLBW xmm9, xmm12
	PUNPCKLBW xmm10, xmm6
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	PUNPCKLBW xmm13, xmm4
	PUNPCKLBW xmm5, xmm15
	PSUBW xmm13, xmm5
	MOVDQA [byte rdx + 32], xmm9
	MOVDQA [byte rdx + 48], xmm13
	ADD rdx, 64
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8sV8s_V16s_Nehalem
_yepCore_Subtract_V8sV8s_V16s_Nehalem:
%else
section .text
global __yepCore_Subtract_V8sV8s_V16s_Nehalem
__yepCore_Subtract_V8sV8s_V16s_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	PMOVSXBW xmm0, [rdi]
	PMOVSXBW xmm8, [byte rdi + 8]
	PMOVSXBW xmm1, [rsi]
	PMOVSXBW xmm9, [byte rdi + 16]
	PMOVSXBW xmm13, [byte rsi + 8]
	PMOVSXBW xmm12, [byte rdi + 24]
	PMOVSXBW xmm10, [byte rsi + 16]
	PMOVSXBW xmm14, [byte rdi + 32]
	PMOVSXBW xmm3, [byte rsi + 24]
	PMOVSXBW xmm11, [byte rdi + 40]
	PMOVSXBW xmm6, [byte rsi + 32]
	PMOVSXBW xmm4, [byte rdi + 48]
	PMOVSXBW xmm7, [byte rsi + 40]
	PSUBW xmm0, xmm1
	PMOVSXBW xmm2, [byte rdi + 56]
	PMOVSXBW xmm5, [byte rsi + 48]
	PSUBW xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVSXBW xmm15, [byte rsi + 56]
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	PMOVSXBW xmm0, [rdi]
	ADD rsi, 64
	PSUBW xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PMOVSXBW xmm8, [byte rdi + 8]
	PMOVSXBW xmm1, [rsi]
	PSUBW xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PMOVSXBW xmm9, [byte rdi + 16]
	PMOVSXBW xmm13, [byte rsi + 8]
	PSUBW xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PMOVSXBW xmm12, [byte rdi + 24]
	PMOVSXBW xmm10, [byte rsi + 16]
	PSUBW xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PMOVSXBW xmm14, [byte rdi + 32]
	PMOVSXBW xmm3, [byte rsi + 24]
	PSUBW xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	PMOVSXBW xmm11, [byte rdi + 40]
	PMOVSXBW xmm6, [byte rsi + 32]
	MOVDQA [byte rdx + 112], xmm2
	PMOVSXBW xmm4, [byte rdi + 48]
	PMOVSXBW xmm7, [byte rsi + 40]
	PSUBW xmm0, xmm1
	ADD rdx, 128
	PMOVSXBW xmm2, [byte rdi + 56]
	PMOVSXBW xmm5, [byte rsi + 48]
	PSUBW xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVSXBW xmm15, [byte rsi + 56]
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	PSUBW xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBW xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBW xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBW xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBW xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8sV8s_V16s_SandyBridge
_yepCore_Subtract_V8sV8s_V16s_SandyBridge:
%else
section .text
global __yepCore_Subtract_V8sV8s_V16s_SandyBridge
__yepCore_Subtract_V8sV8s_V16s_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVSXBW xmm0, [rdi]
	VPMOVSXBW xmm8, [byte rdi + 8]
	VPMOVSXBW xmm1, [rsi]
	VPMOVSXBW xmm9, [byte rdi + 16]
	VPMOVSXBW xmm13, [byte rsi + 8]
	VPMOVSXBW xmm12, [byte rdi + 24]
	VPMOVSXBW xmm10, [byte rsi + 16]
	VPMOVSXBW xmm14, [byte rdi + 32]
	VPMOVSXBW xmm3, [byte rsi + 24]
	VPMOVSXBW xmm11, [byte rdi + 40]
	VPMOVSXBW xmm6, [byte rsi + 32]
	VPMOVSXBW xmm4, [byte rdi + 48]
	VPMOVSXBW xmm7, [byte rsi + 40]
	VPSUBW xmm0, xmm0, xmm1
	VPMOVSXBW xmm2, [byte rdi + 56]
	VPMOVSXBW xmm5, [byte rsi + 48]
	VPSUBW xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVSXBW xmm15, [byte rsi + 56]
	VPSUBW xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVSXBW xmm0, [rdi]
	ADD rsi, 64
	VPSUBW xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPMOVSXBW xmm8, [byte rdi + 8]
	VPMOVSXBW xmm1, [rsi]
	VPSUBW xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPMOVSXBW xmm9, [byte rdi + 16]
	VPMOVSXBW xmm13, [byte rsi + 8]
	VPSUBW xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPMOVSXBW xmm12, [byte rdi + 24]
	VPMOVSXBW xmm10, [byte rsi + 16]
	VPSUBW xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPMOVSXBW xmm14, [byte rdi + 32]
	VPMOVSXBW xmm3, [byte rsi + 24]
	VPSUBW xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VPMOVSXBW xmm11, [byte rdi + 40]
	VPMOVSXBW xmm6, [byte rsi + 32]
	VMOVDQA [byte rdx + 112], xmm2
	VPMOVSXBW xmm4, [byte rdi + 48]
	VPMOVSXBW xmm7, [byte rsi + 40]
	VPSUBW xmm0, xmm0, xmm1
	ADD rdx, 128
	VPMOVSXBW xmm2, [byte rdi + 56]
	VPMOVSXBW xmm5, [byte rsi + 48]
	VPSUBW xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVSXBW xmm15, [byte rsi + 56]
	VPSUBW xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	VPSUBW xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBW xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBW xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBW xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBW xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8sV8s_V16s_Haswell
_yepCore_Subtract_V8sV8s_V16s_Haswell:
%else
section .text
global __yepCore_Subtract_V8sV8s_V16s_Haswell
__yepCore_Subtract_V8sV8s_V16s_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 128
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVSXBW ymm0, [rdi]
	VPMOVSXBW ymm8, [byte rdi + 16]
	VPMOVSXBW ymm7, [rsi]
	VPMOVSXBW ymm9, [byte rdi + 32]
	VPMOVSXBW ymm5, [byte rsi + 16]
	VPMOVSXBW ymm1, [byte rdi + 48]
	VPMOVSXBW ymm3, [byte rsi + 32]
	VPMOVSXBW ymm14, [byte rdi + 64]
	VPMOVSXBW ymm2, [byte rsi + 48]
	VPMOVSXBW ymm12, [byte rdi + 80]
	VPMOVSXBW ymm15, [byte rsi + 64]
	VPMOVSXBW ymm11, [byte rdi + 96]
	VPMOVSXBW ymm10, [byte rsi + 80]
	VPSUBW ymm0, ymm0, ymm7
	VPMOVSXBW ymm13, [byte rdi + 112]
	VPMOVSXBW ymm4, [byte rsi + 96]
	VPSUBW ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVSXBW ymm6, [byte rsi + 112]
	VPSUBW ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 128
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVSXBW ymm0, [rdi]
	ADD rsi, 128
	VPSUBW ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPMOVSXBW ymm8, [byte rdi + 16]
	VPMOVSXBW ymm7, [rsi]
	VPSUBW ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPMOVSXBW ymm9, [byte rdi + 32]
	VPMOVSXBW ymm5, [byte rsi + 16]
	VPSUBW ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPMOVSXBW ymm1, [byte rdi + 48]
	VPMOVSXBW ymm3, [byte rsi + 32]
	VPSUBW ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPMOVSXBW ymm14, [byte rdi + 64]
	VPMOVSXBW ymm2, [byte rsi + 48]
	VPSUBW ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VPMOVSXBW ymm12, [byte rdi + 80]
	VPMOVSXBW ymm15, [byte rsi + 64]
	VMOVDQA [dword rdx + 224], ymm13
	VPMOVSXBW ymm11, [byte rdi + 96]
	VPMOVSXBW ymm10, [byte rsi + 80]
	VPSUBW ymm0, ymm0, ymm7
	ADD rdx, 256
	VPMOVSXBW ymm13, [byte rdi + 112]
	VPMOVSXBW ymm4, [byte rsi + 96]
	VPSUBW ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVSXBW ymm6, [byte rsi + 112]
	VPSUBW ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 128
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBW ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBW ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBW ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBW ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBW ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 128
	JZ .return_ok
	.process_single:
	MOVSX eax, byte [rdi]
	ADD rdi, 1
	MOVSX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.K10 progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8uV8u_V16u_K10
_yepCore_Subtract_V8uV8u_V16u_K10:
%else
section .text
global __yepCore_Subtract_V8uV8u_V16u_K10
__yepCore_Subtract_V8uV8u_V16u_K10:
%endif
	.ENTRY:
	PXOR xmm3, xmm3
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVZX eax, byte [rdi]
	ADD rdi, 1
	MOVZX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 56
	JB .batch_process_finish
	.process_batch_prologue:
	MOVQ xmm2, [rdi]
	MOVQ xmm1, [rsi]
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	MOVQ xmm15, [byte rdi + 24]
	MOVQ xmm13, [byte rsi + 24]
	PUNPCKLBW xmm2, xmm3
	MOVQ xmm7, [byte rdi + 32]
	MOVQ xmm4, [byte rsi + 32]
	PUNPCKLBW xmm8, xmm3
	PUNPCKLBW xmm1, xmm3
	MOVQ xmm12, [byte rdi + 40]
	MOVQ xmm11, [byte rsi + 40]
	PUNPCKLBW xmm9, xmm3
	PUNPCKLBW xmm14, xmm3
	PSUBW xmm2, xmm1
	MOVQ xmm5, [byte rdi + 48]
	MOVQ xmm6, [byte rsi + 48]
	PUNPCKLBW xmm15, xmm3
	PUNPCKLBW xmm10, xmm3
	PSUBW xmm8, xmm14
	MOVDQA [rdx], xmm2
	ADD rdi, 56
	ADD rsi, 56
	PUNPCKLBW xmm7, xmm3
	PUNPCKLBW xmm13, xmm3
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 56
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVQ xmm2, [rdi]
	MOVQ xmm1, [rsi]
	PUNPCKLBW xmm12, xmm3
	PUNPCKLBW xmm4, xmm3
	PSUBW xmm15, xmm13
	MOVDQA [byte rdx + 32], xmm9
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PUNPCKLBW xmm5, xmm3
	PUNPCKLBW xmm11, xmm3
	PSUBW xmm7, xmm4
	MOVDQA [byte rdx + 48], xmm15
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PUNPCKLBW xmm6, xmm3
	PSUBW xmm12, xmm11
	MOVDQA [byte rdx + 64], xmm7
	MOVQ xmm15, [byte rdi + 24]
	MOVQ xmm13, [byte rsi + 24]
	PUNPCKLBW xmm2, xmm3
	PSUBW xmm5, xmm6
	MOVDQA [byte rdx + 80], xmm12
	MOVQ xmm7, [byte rdi + 32]
	MOVQ xmm4, [byte rsi + 32]
	PUNPCKLBW xmm8, xmm3
	PUNPCKLBW xmm1, xmm3
	MOVDQA [byte rdx + 96], xmm5
	MOVQ xmm12, [byte rdi + 40]
	MOVQ xmm11, [byte rsi + 40]
	PUNPCKLBW xmm9, xmm3
	PUNPCKLBW xmm14, xmm3
	PSUBW xmm2, xmm1
	ADD rdx, 112
	MOVQ xmm5, [byte rdi + 48]
	MOVQ xmm6, [byte rsi + 48]
	PUNPCKLBW xmm15, xmm3
	PUNPCKLBW xmm10, xmm3
	PSUBW xmm8, xmm14
	MOVDQA [rdx], xmm2
	ADD rdi, 56
	ADD rsi, 56
	PUNPCKLBW xmm7, xmm3
	PUNPCKLBW xmm13, xmm3
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 56
	JAE .process_batch
	.process_batch_epilogue:
	PUNPCKLBW xmm12, xmm3
	PUNPCKLBW xmm4, xmm3
	PSUBW xmm15, xmm13
	MOVDQA [byte rdx + 32], xmm9
	PUNPCKLBW xmm5, xmm3
	PUNPCKLBW xmm11, xmm3
	PSUBW xmm7, xmm4
	MOVDQA [byte rdx + 48], xmm15
	PUNPCKLBW xmm6, xmm3
	PSUBW xmm12, xmm11
	MOVDQA [byte rdx + 64], xmm7
	PSUBW xmm5, xmm6
	MOVDQA [byte rdx + 80], xmm12
	MOVDQA [byte rdx + 96], xmm5
	ADD rdx, 112
	.batch_process_finish:
	ADD rcx, 56
	JZ .return_ok
	.process_single:
	MOVZX eax, byte [rdi]
	ADD rdi, 1
	MOVZX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8uV8u_V16u_Nehalem
_yepCore_Subtract_V8uV8u_V16u_Nehalem:
%else
section .text
global __yepCore_Subtract_V8uV8u_V16u_Nehalem
__yepCore_Subtract_V8uV8u_V16u_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVZX eax, byte [rdi]
	ADD rdi, 1
	MOVZX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	PMOVZXBW xmm0, [rdi]
	PMOVZXBW xmm8, [byte rdi + 8]
	PMOVZXBW xmm1, [rsi]
	PMOVZXBW xmm9, [byte rdi + 16]
	PMOVZXBW xmm13, [byte rsi + 8]
	PMOVZXBW xmm12, [byte rdi + 24]
	PMOVZXBW xmm10, [byte rsi + 16]
	PMOVZXBW xmm14, [byte rdi + 32]
	PMOVZXBW xmm3, [byte rsi + 24]
	PMOVZXBW xmm11, [byte rdi + 40]
	PMOVZXBW xmm6, [byte rsi + 32]
	PMOVZXBW xmm4, [byte rdi + 48]
	PMOVZXBW xmm7, [byte rsi + 40]
	PSUBW xmm0, xmm1
	PMOVZXBW xmm2, [byte rdi + 56]
	PMOVZXBW xmm5, [byte rsi + 48]
	PSUBW xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVZXBW xmm15, [byte rsi + 56]
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	PMOVZXBW xmm0, [rdi]
	ADD rsi, 64
	PSUBW xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PMOVZXBW xmm8, [byte rdi + 8]
	PMOVZXBW xmm1, [rsi]
	PSUBW xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PMOVZXBW xmm9, [byte rdi + 16]
	PMOVZXBW xmm13, [byte rsi + 8]
	PSUBW xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PMOVZXBW xmm12, [byte rdi + 24]
	PMOVZXBW xmm10, [byte rsi + 16]
	PSUBW xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PMOVZXBW xmm14, [byte rdi + 32]
	PMOVZXBW xmm3, [byte rsi + 24]
	PSUBW xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	PMOVZXBW xmm11, [byte rdi + 40]
	PMOVZXBW xmm6, [byte rsi + 32]
	MOVDQA [byte rdx + 112], xmm2
	PMOVZXBW xmm4, [byte rdi + 48]
	PMOVZXBW xmm7, [byte rsi + 40]
	PSUBW xmm0, xmm1
	ADD rdx, 128
	PMOVZXBW xmm2, [byte rdi + 56]
	PMOVZXBW xmm5, [byte rsi + 48]
	PSUBW xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVZXBW xmm15, [byte rsi + 56]
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	PSUBW xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBW xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBW xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBW xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBW xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOVZX eax, byte [rdi]
	ADD rdi, 1
	MOVZX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8uV8u_V16u_SandyBridge
_yepCore_Subtract_V8uV8u_V16u_SandyBridge:
%else
section .text
global __yepCore_Subtract_V8uV8u_V16u_SandyBridge
__yepCore_Subtract_V8uV8u_V16u_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVZX eax, byte [rdi]
	ADD rdi, 1
	MOVZX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVZXBW xmm0, [rdi]
	VPMOVZXBW xmm8, [byte rdi + 8]
	VPMOVZXBW xmm1, [rsi]
	VPMOVZXBW xmm9, [byte rdi + 16]
	VPMOVZXBW xmm13, [byte rsi + 8]
	VPMOVZXBW xmm12, [byte rdi + 24]
	VPMOVZXBW xmm10, [byte rsi + 16]
	VPMOVZXBW xmm14, [byte rdi + 32]
	VPMOVZXBW xmm3, [byte rsi + 24]
	VPMOVZXBW xmm11, [byte rdi + 40]
	VPMOVZXBW xmm6, [byte rsi + 32]
	VPMOVZXBW xmm4, [byte rdi + 48]
	VPMOVZXBW xmm7, [byte rsi + 40]
	VPSUBW xmm0, xmm0, xmm1
	VPMOVZXBW xmm2, [byte rdi + 56]
	VPMOVZXBW xmm5, [byte rsi + 48]
	VPSUBW xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVZXBW xmm15, [byte rsi + 56]
	VPSUBW xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVZXBW xmm0, [rdi]
	ADD rsi, 64
	VPSUBW xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPMOVZXBW xmm8, [byte rdi + 8]
	VPMOVZXBW xmm1, [rsi]
	VPSUBW xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPMOVZXBW xmm9, [byte rdi + 16]
	VPMOVZXBW xmm13, [byte rsi + 8]
	VPSUBW xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPMOVZXBW xmm12, [byte rdi + 24]
	VPMOVZXBW xmm10, [byte rsi + 16]
	VPSUBW xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPMOVZXBW xmm14, [byte rdi + 32]
	VPMOVZXBW xmm3, [byte rsi + 24]
	VPSUBW xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VPMOVZXBW xmm11, [byte rdi + 40]
	VPMOVZXBW xmm6, [byte rsi + 32]
	VMOVDQA [byte rdx + 112], xmm2
	VPMOVZXBW xmm4, [byte rdi + 48]
	VPMOVZXBW xmm7, [byte rsi + 40]
	VPSUBW xmm0, xmm0, xmm1
	ADD rdx, 128
	VPMOVZXBW xmm2, [byte rdi + 56]
	VPMOVZXBW xmm5, [byte rsi + 48]
	VPSUBW xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVZXBW xmm15, [byte rsi + 56]
	VPSUBW xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	VPSUBW xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBW xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBW xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBW xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBW xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOVZX eax, byte [rdi]
	ADD rdi, 1
	MOVZX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V8uV8u_V16u_Haswell
_yepCore_Subtract_V8uV8u_V16u_Haswell:
%else
section .text
global __yepCore_Subtract_V8uV8u_V16u_Haswell
__yepCore_Subtract_V8uV8u_V16u_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOVZX eax, byte [rdi]
	ADD rdi, 1
	MOVZX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 128
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVZXBW ymm0, [rdi]
	VPMOVZXBW ymm8, [byte rdi + 16]
	VPMOVZXBW ymm7, [rsi]
	VPMOVZXBW ymm9, [byte rdi + 32]
	VPMOVZXBW ymm5, [byte rsi + 16]
	VPMOVZXBW ymm1, [byte rdi + 48]
	VPMOVZXBW ymm3, [byte rsi + 32]
	VPMOVZXBW ymm14, [byte rdi + 64]
	VPMOVZXBW ymm2, [byte rsi + 48]
	VPMOVZXBW ymm12, [byte rdi + 80]
	VPMOVZXBW ymm15, [byte rsi + 64]
	VPMOVZXBW ymm11, [byte rdi + 96]
	VPMOVZXBW ymm10, [byte rsi + 80]
	VPSUBW ymm0, ymm0, ymm7
	VPMOVZXBW ymm13, [byte rdi + 112]
	VPMOVZXBW ymm4, [byte rsi + 96]
	VPSUBW ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVZXBW ymm6, [byte rsi + 112]
	VPSUBW ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 128
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVZXBW ymm0, [rdi]
	ADD rsi, 128
	VPSUBW ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPMOVZXBW ymm8, [byte rdi + 16]
	VPMOVZXBW ymm7, [rsi]
	VPSUBW ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPMOVZXBW ymm9, [byte rdi + 32]
	VPMOVZXBW ymm5, [byte rsi + 16]
	VPSUBW ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPMOVZXBW ymm1, [byte rdi + 48]
	VPMOVZXBW ymm3, [byte rsi + 32]
	VPSUBW ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPMOVZXBW ymm14, [byte rdi + 64]
	VPMOVZXBW ymm2, [byte rsi + 48]
	VPSUBW ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VPMOVZXBW ymm12, [byte rdi + 80]
	VPMOVZXBW ymm15, [byte rsi + 64]
	VMOVDQA [dword rdx + 224], ymm13
	VPMOVZXBW ymm11, [byte rdi + 96]
	VPMOVZXBW ymm10, [byte rsi + 80]
	VPSUBW ymm0, ymm0, ymm7
	ADD rdx, 256
	VPMOVZXBW ymm13, [byte rdi + 112]
	VPMOVZXBW ymm4, [byte rsi + 96]
	VPSUBW ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVZXBW ymm6, [byte rsi + 112]
	VPSUBW ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 128
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBW ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBW ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBW ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBW ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBW ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 128
	JZ .return_ok
	.process_single:
	MOVZX eax, byte [rdi]
	ADD rdi, 1
	MOVZX r10d, byte [rsi]
	ADD rsi, 1
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16sV16s_V16s_Nehalem
_yepCore_Subtract_V16sV16s_V16s_Nehalem:
%else
section .text
global __yepCore_Subtract_V16sV16s_V16s_Nehalem
__yepCore_Subtract_V16sV16s_V16s_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	MOVDQU xmm0, [rdi]
	MOVDQU xmm8, [byte rdi + 16]
	MOVDQU xmm1, [rsi]
	MOVDQU xmm9, [byte rdi + 32]
	MOVDQU xmm13, [byte rsi + 16]
	MOVDQU xmm12, [byte rdi + 48]
	MOVDQU xmm10, [byte rsi + 32]
	MOVDQU xmm14, [byte rdi + 64]
	MOVDQU xmm3, [byte rsi + 48]
	MOVDQU xmm11, [byte rdi + 80]
	MOVDQU xmm6, [byte rsi + 64]
	MOVDQU xmm4, [byte rdi + 96]
	MOVDQU xmm7, [byte rsi + 80]
	PSUBW xmm0, xmm1
	MOVDQU xmm2, [byte rdi + 112]
	MOVDQU xmm5, [byte rsi + 96]
	PSUBW xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 128
	MOVDQU xmm15, [byte rsi + 112]
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVDQU xmm0, [rdi]
	ADD rsi, 128
	PSUBW xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	MOVDQU xmm8, [byte rdi + 16]
	MOVDQU xmm1, [rsi]
	PSUBW xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	MOVDQU xmm9, [byte rdi + 32]
	MOVDQU xmm13, [byte rsi + 16]
	PSUBW xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	MOVDQU xmm12, [byte rdi + 48]
	MOVDQU xmm10, [byte rsi + 32]
	PSUBW xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	MOVDQU xmm14, [byte rdi + 64]
	MOVDQU xmm3, [byte rsi + 48]
	PSUBW xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQU xmm11, [byte rdi + 80]
	MOVDQU xmm6, [byte rsi + 64]
	MOVDQA [byte rdx + 112], xmm2
	MOVDQU xmm4, [byte rdi + 96]
	MOVDQU xmm7, [byte rsi + 80]
	PSUBW xmm0, xmm1
	ADD rdx, 128
	MOVDQU xmm2, [byte rdi + 112]
	MOVDQU xmm5, [byte rsi + 96]
	PSUBW xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 128
	MOVDQU xmm15, [byte rsi + 112]
	PSUBW xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	PSUBW xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBW xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBW xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBW xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBW xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16sV16s_V16s_SandyBridge
_yepCore_Subtract_V16sV16s_V16s_SandyBridge:
%else
section .text
global __yepCore_Subtract_V16sV16s_V16s_SandyBridge
__yepCore_Subtract_V16sV16s_V16s_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVDQU xmm0, [rdi]
	VMOVDQU xmm8, [byte rdi + 16]
	VMOVDQU xmm1, [rsi]
	VMOVDQU xmm9, [byte rdi + 32]
	VMOVDQU xmm13, [byte rsi + 16]
	VMOVDQU xmm12, [byte rdi + 48]
	VMOVDQU xmm10, [byte rsi + 32]
	VMOVDQU xmm14, [byte rdi + 64]
	VMOVDQU xmm3, [byte rsi + 48]
	VMOVDQU xmm11, [byte rdi + 80]
	VMOVDQU xmm6, [byte rsi + 64]
	VMOVDQU xmm4, [byte rdi + 96]
	VMOVDQU xmm7, [byte rsi + 80]
	VPSUBW xmm0, xmm0, xmm1
	VMOVDQU xmm2, [byte rdi + 112]
	VMOVDQU xmm5, [byte rsi + 96]
	VPSUBW xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 128
	VMOVDQU xmm15, [byte rsi + 112]
	VPSUBW xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVDQU xmm0, [rdi]
	ADD rsi, 128
	VPSUBW xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VMOVDQU xmm8, [byte rdi + 16]
	VMOVDQU xmm1, [rsi]
	VPSUBW xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VMOVDQU xmm9, [byte rdi + 32]
	VMOVDQU xmm13, [byte rsi + 16]
	VPSUBW xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VMOVDQU xmm12, [byte rdi + 48]
	VMOVDQU xmm10, [byte rsi + 32]
	VPSUBW xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VMOVDQU xmm14, [byte rdi + 64]
	VMOVDQU xmm3, [byte rsi + 48]
	VPSUBW xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQU xmm11, [byte rdi + 80]
	VMOVDQU xmm6, [byte rsi + 64]
	VMOVDQA [byte rdx + 112], xmm2
	VMOVDQU xmm4, [byte rdi + 96]
	VMOVDQU xmm7, [byte rsi + 80]
	VPSUBW xmm0, xmm0, xmm1
	ADD rdx, 128
	VMOVDQU xmm2, [byte rdi + 112]
	VMOVDQU xmm5, [byte rsi + 96]
	VPSUBW xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 128
	VMOVDQU xmm15, [byte rsi + 112]
	VPSUBW xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBW xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBW xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBW xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBW xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBW xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16sV16s_V16s_Haswell
_yepCore_Subtract_V16sV16s_V16s_Haswell:
%else
section .text
global __yepCore_Subtract_V16sV16s_V16s_Haswell
__yepCore_Subtract_V16sV16s_V16s_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 1
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 128
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVDQU ymm0, [rdi]
	VMOVDQU ymm8, [byte rdi + 32]
	VMOVDQU ymm7, [rsi]
	VMOVDQU ymm9, [byte rdi + 64]
	VMOVDQU ymm5, [byte rsi + 32]
	VMOVDQU ymm1, [byte rdi + 96]
	VMOVDQU ymm3, [byte rsi + 64]
	VMOVDQU ymm14, [dword rdi + 128]
	VMOVDQU ymm2, [byte rsi + 96]
	VMOVDQU ymm12, [dword rdi + 160]
	VMOVDQU ymm15, [dword rsi + 128]
	VMOVDQU ymm11, [dword rdi + 192]
	VMOVDQU ymm10, [dword rsi + 160]
	VPSUBW ymm0, ymm0, ymm7
	VMOVDQU ymm13, [dword rdi + 224]
	VMOVDQU ymm4, [dword rsi + 192]
	VPSUBW ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 256
	VMOVDQU ymm6, [dword rsi + 224]
	VPSUBW ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 128
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVDQU ymm0, [rdi]
	ADD rsi, 256
	VPSUBW ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VMOVDQU ymm8, [byte rdi + 32]
	VMOVDQU ymm7, [rsi]
	VPSUBW ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VMOVDQU ymm9, [byte rdi + 64]
	VMOVDQU ymm5, [byte rsi + 32]
	VPSUBW ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VMOVDQU ymm1, [byte rdi + 96]
	VMOVDQU ymm3, [byte rsi + 64]
	VPSUBW ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VMOVDQU ymm14, [dword rdi + 128]
	VMOVDQU ymm2, [byte rsi + 96]
	VPSUBW ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQU ymm12, [dword rdi + 160]
	VMOVDQU ymm15, [dword rsi + 128]
	VMOVDQA [dword rdx + 224], ymm13
	VMOVDQU ymm11, [dword rdi + 192]
	VMOVDQU ymm10, [dword rsi + 160]
	VPSUBW ymm0, ymm0, ymm7
	ADD rdx, 256
	VMOVDQU ymm13, [dword rdi + 224]
	VMOVDQU ymm4, [dword rsi + 192]
	VPSUBW ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 256
	VMOVDQU ymm6, [dword rsi + 224]
	VPSUBW ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 128
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 256
	VPSUBW ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBW ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBW ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBW ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBW ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 128
	JZ .return_ok
	.process_single:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], ax
	ADD rdx, 2
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.K10 progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16sV16s_V32s_K10
_yepCore_Subtract_V16sV16s_V32s_K10:
%else
section .text
global __yepCore_Subtract_V16sV16s_V32s_K10
__yepCore_Subtract_V16sV16s_V32s_K10:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 16
	JB .batch_process_finish
	.process_batch_prologue:
	MOVQ xmm3, [rdi]
	MOVQ xmm2, [rsi]
	PXOR xmm1, xmm1
	PXOR xmm0, xmm0
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PXOR xmm7, xmm7
	PXOR xmm11, xmm11
	PCMPGTW xmm1, xmm3
	PCMPGTW xmm0, xmm2
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PXOR xmm12, xmm12
	PXOR xmm6, xmm6
	PCMPGTW xmm7, xmm8
	PCMPGTW xmm11, xmm14
	PUNPCKLWD xmm3, xmm1
	PUNPCKLWD xmm2, xmm0
	PSUBD xmm3, xmm2
	MOVQ xmm13, [byte rdi + 24]
	MOVQ xmm5, [byte rsi + 24]
	PXOR xmm4, xmm4
	PXOR xmm15, xmm15
	PCMPGTW xmm12, xmm9
	PCMPGTW xmm6, xmm10
	PUNPCKLWD xmm8, xmm7
	PUNPCKLWD xmm14, xmm11
	PSUBD xmm8, xmm14
	MOVDQA [rdx], xmm3
	ADD rdi, 32
	ADD rsi, 32
	PCMPGTW xmm4, xmm13
	PCMPGTW xmm15, xmm5
	PUNPCKLWD xmm9, xmm12
	PUNPCKLWD xmm10, xmm6
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVQ xmm3, [rdi]
	MOVQ xmm2, [rsi]
	PXOR xmm1, xmm1
	PXOR xmm0, xmm0
	PUNPCKLWD xmm13, xmm4
	PUNPCKLWD xmm5, xmm15
	PSUBD xmm13, xmm5
	MOVDQA [byte rdx + 32], xmm9
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PXOR xmm7, xmm7
	PXOR xmm11, xmm11
	PCMPGTW xmm1, xmm3
	PCMPGTW xmm0, xmm2
	MOVDQA [byte rdx + 48], xmm13
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PXOR xmm12, xmm12
	PXOR xmm6, xmm6
	PCMPGTW xmm7, xmm8
	PCMPGTW xmm11, xmm14
	PUNPCKLWD xmm3, xmm1
	PUNPCKLWD xmm2, xmm0
	PSUBD xmm3, xmm2
	ADD rdx, 64
	MOVQ xmm13, [byte rdi + 24]
	MOVQ xmm5, [byte rsi + 24]
	PXOR xmm4, xmm4
	PXOR xmm15, xmm15
	PCMPGTW xmm12, xmm9
	PCMPGTW xmm6, xmm10
	PUNPCKLWD xmm8, xmm7
	PUNPCKLWD xmm14, xmm11
	PSUBD xmm8, xmm14
	MOVDQA [rdx], xmm3
	ADD rdi, 32
	ADD rsi, 32
	PCMPGTW xmm4, xmm13
	PCMPGTW xmm15, xmm5
	PUNPCKLWD xmm9, xmm12
	PUNPCKLWD xmm10, xmm6
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JAE .process_batch
	.process_batch_epilogue:
	PUNPCKLWD xmm13, xmm4
	PUNPCKLWD xmm5, xmm15
	PSUBD xmm13, xmm5
	MOVDQA [byte rdx + 32], xmm9
	MOVDQA [byte rdx + 48], xmm13
	ADD rdx, 64
	.batch_process_finish:
	ADD rcx, 16
	JZ .return_ok
	.process_single:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16sV16s_V32s_Nehalem
_yepCore_Subtract_V16sV16s_V32s_Nehalem:
%else
section .text
global __yepCore_Subtract_V16sV16s_V32s_Nehalem
__yepCore_Subtract_V16sV16s_V32s_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	PMOVSXWD xmm0, [rdi]
	PMOVSXWD xmm8, [byte rdi + 8]
	PMOVSXWD xmm1, [rsi]
	PMOVSXWD xmm9, [byte rdi + 16]
	PMOVSXWD xmm13, [byte rsi + 8]
	PMOVSXWD xmm12, [byte rdi + 24]
	PMOVSXWD xmm10, [byte rsi + 16]
	PMOVSXWD xmm14, [byte rdi + 32]
	PMOVSXWD xmm3, [byte rsi + 24]
	PMOVSXWD xmm11, [byte rdi + 40]
	PMOVSXWD xmm6, [byte rsi + 32]
	PMOVSXWD xmm4, [byte rdi + 48]
	PMOVSXWD xmm7, [byte rsi + 40]
	PSUBD xmm0, xmm1
	PMOVSXWD xmm2, [byte rdi + 56]
	PMOVSXWD xmm5, [byte rsi + 48]
	PSUBD xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVSXWD xmm15, [byte rsi + 56]
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	PMOVSXWD xmm0, [rdi]
	ADD rsi, 64
	PSUBD xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PMOVSXWD xmm8, [byte rdi + 8]
	PMOVSXWD xmm1, [rsi]
	PSUBD xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PMOVSXWD xmm9, [byte rdi + 16]
	PMOVSXWD xmm13, [byte rsi + 8]
	PSUBD xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PMOVSXWD xmm12, [byte rdi + 24]
	PMOVSXWD xmm10, [byte rsi + 16]
	PSUBD xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PMOVSXWD xmm14, [byte rdi + 32]
	PMOVSXWD xmm3, [byte rsi + 24]
	PSUBD xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	PMOVSXWD xmm11, [byte rdi + 40]
	PMOVSXWD xmm6, [byte rsi + 32]
	MOVDQA [byte rdx + 112], xmm2
	PMOVSXWD xmm4, [byte rdi + 48]
	PMOVSXWD xmm7, [byte rsi + 40]
	PSUBD xmm0, xmm1
	ADD rdx, 128
	PMOVSXWD xmm2, [byte rdi + 56]
	PMOVSXWD xmm5, [byte rsi + 48]
	PSUBD xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVSXWD xmm15, [byte rsi + 56]
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	PSUBD xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBD xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBD xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBD xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBD xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16sV16s_V32s_SandyBridge
_yepCore_Subtract_V16sV16s_V32s_SandyBridge:
%else
section .text
global __yepCore_Subtract_V16sV16s_V32s_SandyBridge
__yepCore_Subtract_V16sV16s_V32s_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVSXWD xmm0, [rdi]
	VPMOVSXWD xmm8, [byte rdi + 8]
	VPMOVSXWD xmm1, [rsi]
	VPMOVSXWD xmm9, [byte rdi + 16]
	VPMOVSXWD xmm13, [byte rsi + 8]
	VPMOVSXWD xmm12, [byte rdi + 24]
	VPMOVSXWD xmm10, [byte rsi + 16]
	VPMOVSXWD xmm14, [byte rdi + 32]
	VPMOVSXWD xmm3, [byte rsi + 24]
	VPMOVSXWD xmm11, [byte rdi + 40]
	VPMOVSXWD xmm6, [byte rsi + 32]
	VPMOVSXWD xmm4, [byte rdi + 48]
	VPMOVSXWD xmm7, [byte rsi + 40]
	VPSUBD xmm0, xmm0, xmm1
	VPMOVSXWD xmm2, [byte rdi + 56]
	VPMOVSXWD xmm5, [byte rsi + 48]
	VPSUBD xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVSXWD xmm15, [byte rsi + 56]
	VPSUBD xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVSXWD xmm0, [rdi]
	ADD rsi, 64
	VPSUBD xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPMOVSXWD xmm8, [byte rdi + 8]
	VPMOVSXWD xmm1, [rsi]
	VPSUBD xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPMOVSXWD xmm9, [byte rdi + 16]
	VPMOVSXWD xmm13, [byte rsi + 8]
	VPSUBD xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPMOVSXWD xmm12, [byte rdi + 24]
	VPMOVSXWD xmm10, [byte rsi + 16]
	VPSUBD xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPMOVSXWD xmm14, [byte rdi + 32]
	VPMOVSXWD xmm3, [byte rsi + 24]
	VPSUBD xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VPMOVSXWD xmm11, [byte rdi + 40]
	VPMOVSXWD xmm6, [byte rsi + 32]
	VMOVDQA [byte rdx + 112], xmm2
	VPMOVSXWD xmm4, [byte rdi + 48]
	VPMOVSXWD xmm7, [byte rsi + 40]
	VPSUBD xmm0, xmm0, xmm1
	ADD rdx, 128
	VPMOVSXWD xmm2, [byte rdi + 56]
	VPMOVSXWD xmm5, [byte rsi + 48]
	VPSUBD xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVSXWD xmm15, [byte rsi + 56]
	VPSUBD xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	VPSUBD xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBD xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBD xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBD xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBD xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16sV16s_V32s_Haswell
_yepCore_Subtract_V16sV16s_V32s_Haswell:
%else
section .text
global __yepCore_Subtract_V16sV16s_V32s_Haswell
__yepCore_Subtract_V16sV16s_V32s_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVSXWD ymm0, [rdi]
	VPMOVSXWD ymm8, [byte rdi + 16]
	VPMOVSXWD ymm7, [rsi]
	VPMOVSXWD ymm9, [byte rdi + 32]
	VPMOVSXWD ymm5, [byte rsi + 16]
	VPMOVSXWD ymm1, [byte rdi + 48]
	VPMOVSXWD ymm3, [byte rsi + 32]
	VPMOVSXWD ymm14, [byte rdi + 64]
	VPMOVSXWD ymm2, [byte rsi + 48]
	VPMOVSXWD ymm12, [byte rdi + 80]
	VPMOVSXWD ymm15, [byte rsi + 64]
	VPMOVSXWD ymm11, [byte rdi + 96]
	VPMOVSXWD ymm10, [byte rsi + 80]
	VPSUBD ymm0, ymm0, ymm7
	VPMOVSXWD ymm13, [byte rdi + 112]
	VPMOVSXWD ymm4, [byte rsi + 96]
	VPSUBD ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVSXWD ymm6, [byte rsi + 112]
	VPSUBD ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVSXWD ymm0, [rdi]
	ADD rsi, 128
	VPSUBD ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPMOVSXWD ymm8, [byte rdi + 16]
	VPMOVSXWD ymm7, [rsi]
	VPSUBD ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPMOVSXWD ymm9, [byte rdi + 32]
	VPMOVSXWD ymm5, [byte rsi + 16]
	VPSUBD ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPMOVSXWD ymm1, [byte rdi + 48]
	VPMOVSXWD ymm3, [byte rsi + 32]
	VPSUBD ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPMOVSXWD ymm14, [byte rdi + 64]
	VPMOVSXWD ymm2, [byte rsi + 48]
	VPSUBD ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VPMOVSXWD ymm12, [byte rdi + 80]
	VPMOVSXWD ymm15, [byte rsi + 64]
	VMOVDQA [dword rdx + 224], ymm13
	VPMOVSXWD ymm11, [byte rdi + 96]
	VPMOVSXWD ymm10, [byte rsi + 80]
	VPSUBD ymm0, ymm0, ymm7
	ADD rdx, 256
	VPMOVSXWD ymm13, [byte rdi + 112]
	VPMOVSXWD ymm4, [byte rsi + 96]
	VPSUBD ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVSXWD ymm6, [byte rsi + 112]
	VPSUBD ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBD ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBD ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBD ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBD ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBD ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOVSX eax, word [rdi]
	ADD rdi, 2
	MOVSX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.K10 progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16uV16u_V32u_K10
_yepCore_Subtract_V16uV16u_V32u_K10:
%else
section .text
global __yepCore_Subtract_V16uV16u_V32u_K10
__yepCore_Subtract_V16uV16u_V32u_K10:
%endif
	.ENTRY:
	PXOR xmm3, xmm3
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVZX eax, word [rdi]
	ADD rdi, 2
	MOVZX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 28
	JB .batch_process_finish
	.process_batch_prologue:
	MOVQ xmm2, [rdi]
	MOVQ xmm1, [rsi]
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	MOVQ xmm15, [byte rdi + 24]
	MOVQ xmm13, [byte rsi + 24]
	PUNPCKLWD xmm2, xmm3
	MOVQ xmm7, [byte rdi + 32]
	MOVQ xmm4, [byte rsi + 32]
	PUNPCKLWD xmm8, xmm3
	PUNPCKLWD xmm1, xmm3
	MOVQ xmm12, [byte rdi + 40]
	MOVQ xmm11, [byte rsi + 40]
	PUNPCKLWD xmm9, xmm3
	PUNPCKLWD xmm14, xmm3
	PSUBD xmm2, xmm1
	MOVQ xmm5, [byte rdi + 48]
	MOVQ xmm6, [byte rsi + 48]
	PUNPCKLWD xmm15, xmm3
	PUNPCKLWD xmm10, xmm3
	PSUBD xmm8, xmm14
	MOVDQA [rdx], xmm2
	ADD rdi, 56
	ADD rsi, 56
	PUNPCKLWD xmm7, xmm3
	PUNPCKLWD xmm13, xmm3
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 28
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVQ xmm2, [rdi]
	MOVQ xmm1, [rsi]
	PUNPCKLWD xmm12, xmm3
	PUNPCKLWD xmm4, xmm3
	PSUBD xmm15, xmm13
	MOVDQA [byte rdx + 32], xmm9
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PUNPCKLWD xmm5, xmm3
	PUNPCKLWD xmm11, xmm3
	PSUBD xmm7, xmm4
	MOVDQA [byte rdx + 48], xmm15
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PUNPCKLWD xmm6, xmm3
	PSUBD xmm12, xmm11
	MOVDQA [byte rdx + 64], xmm7
	MOVQ xmm15, [byte rdi + 24]
	MOVQ xmm13, [byte rsi + 24]
	PUNPCKLWD xmm2, xmm3
	PSUBD xmm5, xmm6
	MOVDQA [byte rdx + 80], xmm12
	MOVQ xmm7, [byte rdi + 32]
	MOVQ xmm4, [byte rsi + 32]
	PUNPCKLWD xmm8, xmm3
	PUNPCKLWD xmm1, xmm3
	MOVDQA [byte rdx + 96], xmm5
	MOVQ xmm12, [byte rdi + 40]
	MOVQ xmm11, [byte rsi + 40]
	PUNPCKLWD xmm9, xmm3
	PUNPCKLWD xmm14, xmm3
	PSUBD xmm2, xmm1
	ADD rdx, 112
	MOVQ xmm5, [byte rdi + 48]
	MOVQ xmm6, [byte rsi + 48]
	PUNPCKLWD xmm15, xmm3
	PUNPCKLWD xmm10, xmm3
	PSUBD xmm8, xmm14
	MOVDQA [rdx], xmm2
	ADD rdi, 56
	ADD rsi, 56
	PUNPCKLWD xmm7, xmm3
	PUNPCKLWD xmm13, xmm3
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 28
	JAE .process_batch
	.process_batch_epilogue:
	PUNPCKLWD xmm12, xmm3
	PUNPCKLWD xmm4, xmm3
	PSUBD xmm15, xmm13
	MOVDQA [byte rdx + 32], xmm9
	PUNPCKLWD xmm5, xmm3
	PUNPCKLWD xmm11, xmm3
	PSUBD xmm7, xmm4
	MOVDQA [byte rdx + 48], xmm15
	PUNPCKLWD xmm6, xmm3
	PSUBD xmm12, xmm11
	MOVDQA [byte rdx + 64], xmm7
	PSUBD xmm5, xmm6
	MOVDQA [byte rdx + 80], xmm12
	MOVDQA [byte rdx + 96], xmm5
	ADD rdx, 112
	.batch_process_finish:
	ADD rcx, 28
	JZ .return_ok
	.process_single:
	MOVZX eax, word [rdi]
	ADD rdi, 2
	MOVZX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16uV16u_V32u_Nehalem
_yepCore_Subtract_V16uV16u_V32u_Nehalem:
%else
section .text
global __yepCore_Subtract_V16uV16u_V32u_Nehalem
__yepCore_Subtract_V16uV16u_V32u_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVZX eax, word [rdi]
	ADD rdi, 2
	MOVZX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	PMOVZXWD xmm0, [rdi]
	PMOVZXWD xmm8, [byte rdi + 8]
	PMOVZXWD xmm1, [rsi]
	PMOVZXWD xmm9, [byte rdi + 16]
	PMOVZXWD xmm13, [byte rsi + 8]
	PMOVZXWD xmm12, [byte rdi + 24]
	PMOVZXWD xmm10, [byte rsi + 16]
	PMOVZXWD xmm14, [byte rdi + 32]
	PMOVZXWD xmm3, [byte rsi + 24]
	PMOVZXWD xmm11, [byte rdi + 40]
	PMOVZXWD xmm6, [byte rsi + 32]
	PMOVZXWD xmm4, [byte rdi + 48]
	PMOVZXWD xmm7, [byte rsi + 40]
	PSUBD xmm0, xmm1
	PMOVZXWD xmm2, [byte rdi + 56]
	PMOVZXWD xmm5, [byte rsi + 48]
	PSUBD xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVZXWD xmm15, [byte rsi + 56]
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	PMOVZXWD xmm0, [rdi]
	ADD rsi, 64
	PSUBD xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PMOVZXWD xmm8, [byte rdi + 8]
	PMOVZXWD xmm1, [rsi]
	PSUBD xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PMOVZXWD xmm9, [byte rdi + 16]
	PMOVZXWD xmm13, [byte rsi + 8]
	PSUBD xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PMOVZXWD xmm12, [byte rdi + 24]
	PMOVZXWD xmm10, [byte rsi + 16]
	PSUBD xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PMOVZXWD xmm14, [byte rdi + 32]
	PMOVZXWD xmm3, [byte rsi + 24]
	PSUBD xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	PMOVZXWD xmm11, [byte rdi + 40]
	PMOVZXWD xmm6, [byte rsi + 32]
	MOVDQA [byte rdx + 112], xmm2
	PMOVZXWD xmm4, [byte rdi + 48]
	PMOVZXWD xmm7, [byte rsi + 40]
	PSUBD xmm0, xmm1
	ADD rdx, 128
	PMOVZXWD xmm2, [byte rdi + 56]
	PMOVZXWD xmm5, [byte rsi + 48]
	PSUBD xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVZXWD xmm15, [byte rsi + 56]
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	PSUBD xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBD xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBD xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBD xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBD xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOVZX eax, word [rdi]
	ADD rdi, 2
	MOVZX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16uV16u_V32u_SandyBridge
_yepCore_Subtract_V16uV16u_V32u_SandyBridge:
%else
section .text
global __yepCore_Subtract_V16uV16u_V32u_SandyBridge
__yepCore_Subtract_V16uV16u_V32u_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVZX eax, word [rdi]
	ADD rdi, 2
	MOVZX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVZXWD xmm0, [rdi]
	VPMOVZXWD xmm8, [byte rdi + 8]
	VPMOVZXWD xmm1, [rsi]
	VPMOVZXWD xmm9, [byte rdi + 16]
	VPMOVZXWD xmm13, [byte rsi + 8]
	VPMOVZXWD xmm12, [byte rdi + 24]
	VPMOVZXWD xmm10, [byte rsi + 16]
	VPMOVZXWD xmm14, [byte rdi + 32]
	VPMOVZXWD xmm3, [byte rsi + 24]
	VPMOVZXWD xmm11, [byte rdi + 40]
	VPMOVZXWD xmm6, [byte rsi + 32]
	VPMOVZXWD xmm4, [byte rdi + 48]
	VPMOVZXWD xmm7, [byte rsi + 40]
	VPSUBD xmm0, xmm0, xmm1
	VPMOVZXWD xmm2, [byte rdi + 56]
	VPMOVZXWD xmm5, [byte rsi + 48]
	VPSUBD xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVZXWD xmm15, [byte rsi + 56]
	VPSUBD xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVZXWD xmm0, [rdi]
	ADD rsi, 64
	VPSUBD xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPMOVZXWD xmm8, [byte rdi + 8]
	VPMOVZXWD xmm1, [rsi]
	VPSUBD xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPMOVZXWD xmm9, [byte rdi + 16]
	VPMOVZXWD xmm13, [byte rsi + 8]
	VPSUBD xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPMOVZXWD xmm12, [byte rdi + 24]
	VPMOVZXWD xmm10, [byte rsi + 16]
	VPSUBD xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPMOVZXWD xmm14, [byte rdi + 32]
	VPMOVZXWD xmm3, [byte rsi + 24]
	VPSUBD xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VPMOVZXWD xmm11, [byte rdi + 40]
	VPMOVZXWD xmm6, [byte rsi + 32]
	VMOVDQA [byte rdx + 112], xmm2
	VPMOVZXWD xmm4, [byte rdi + 48]
	VPMOVZXWD xmm7, [byte rsi + 40]
	VPSUBD xmm0, xmm0, xmm1
	ADD rdx, 128
	VPMOVZXWD xmm2, [byte rdi + 56]
	VPMOVZXWD xmm5, [byte rsi + 48]
	VPSUBD xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVZXWD xmm15, [byte rsi + 56]
	VPSUBD xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	VPSUBD xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBD xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBD xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBD xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBD xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOVZX eax, word [rdi]
	ADD rdi, 2
	MOVZX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V16uV16u_V32u_Haswell
_yepCore_Subtract_V16uV16u_V32u_Haswell:
%else
section .text
global __yepCore_Subtract_V16uV16u_V32u_Haswell
__yepCore_Subtract_V16uV16u_V32u_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 1
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 1
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOVZX eax, word [rdi]
	ADD rdi, 2
	MOVZX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVZXWD ymm0, [rdi]
	VPMOVZXWD ymm8, [byte rdi + 16]
	VPMOVZXWD ymm7, [rsi]
	VPMOVZXWD ymm9, [byte rdi + 32]
	VPMOVZXWD ymm5, [byte rsi + 16]
	VPMOVZXWD ymm1, [byte rdi + 48]
	VPMOVZXWD ymm3, [byte rsi + 32]
	VPMOVZXWD ymm14, [byte rdi + 64]
	VPMOVZXWD ymm2, [byte rsi + 48]
	VPMOVZXWD ymm12, [byte rdi + 80]
	VPMOVZXWD ymm15, [byte rsi + 64]
	VPMOVZXWD ymm11, [byte rdi + 96]
	VPMOVZXWD ymm10, [byte rsi + 80]
	VPSUBD ymm0, ymm0, ymm7
	VPMOVZXWD ymm13, [byte rdi + 112]
	VPMOVZXWD ymm4, [byte rsi + 96]
	VPSUBD ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVZXWD ymm6, [byte rsi + 112]
	VPSUBD ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVZXWD ymm0, [rdi]
	ADD rsi, 128
	VPSUBD ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPMOVZXWD ymm8, [byte rdi + 16]
	VPMOVZXWD ymm7, [rsi]
	VPSUBD ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPMOVZXWD ymm9, [byte rdi + 32]
	VPMOVZXWD ymm5, [byte rsi + 16]
	VPSUBD ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPMOVZXWD ymm1, [byte rdi + 48]
	VPMOVZXWD ymm3, [byte rsi + 32]
	VPSUBD ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPMOVZXWD ymm14, [byte rdi + 64]
	VPMOVZXWD ymm2, [byte rsi + 48]
	VPSUBD ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VPMOVZXWD ymm12, [byte rdi + 80]
	VPMOVZXWD ymm15, [byte rsi + 64]
	VMOVDQA [dword rdx + 224], ymm13
	VPMOVZXWD ymm11, [byte rdi + 96]
	VPMOVZXWD ymm10, [byte rsi + 80]
	VPSUBD ymm0, ymm0, ymm7
	ADD rdx, 256
	VPMOVZXWD ymm13, [byte rdi + 112]
	VPMOVZXWD ymm4, [byte rsi + 96]
	VPSUBD ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVZXWD ymm6, [byte rsi + 112]
	VPSUBD ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBD ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBD ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBD ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBD ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBD ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOVZX eax, word [rdi]
	ADD rdi, 2
	MOVZX r10d, word [rsi]
	ADD rsi, 2
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32sV32s_V32s_Nehalem
_yepCore_Subtract_V32sV32s_V32s_Nehalem:
%else
section .text
global __yepCore_Subtract_V32sV32s_V32s_Nehalem
__yepCore_Subtract_V32sV32s_V32s_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	MOVDQU xmm0, [rdi]
	MOVDQU xmm8, [byte rdi + 16]
	MOVDQU xmm1, [rsi]
	MOVDQU xmm9, [byte rdi + 32]
	MOVDQU xmm13, [byte rsi + 16]
	MOVDQU xmm12, [byte rdi + 48]
	MOVDQU xmm10, [byte rsi + 32]
	MOVDQU xmm14, [byte rdi + 64]
	MOVDQU xmm3, [byte rsi + 48]
	MOVDQU xmm11, [byte rdi + 80]
	MOVDQU xmm6, [byte rsi + 64]
	MOVDQU xmm4, [byte rdi + 96]
	MOVDQU xmm7, [byte rsi + 80]
	PSUBD xmm0, xmm1
	MOVDQU xmm2, [byte rdi + 112]
	MOVDQU xmm5, [byte rsi + 96]
	PSUBD xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 128
	MOVDQU xmm15, [byte rsi + 112]
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVDQU xmm0, [rdi]
	ADD rsi, 128
	PSUBD xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	MOVDQU xmm8, [byte rdi + 16]
	MOVDQU xmm1, [rsi]
	PSUBD xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	MOVDQU xmm9, [byte rdi + 32]
	MOVDQU xmm13, [byte rsi + 16]
	PSUBD xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	MOVDQU xmm12, [byte rdi + 48]
	MOVDQU xmm10, [byte rsi + 32]
	PSUBD xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	MOVDQU xmm14, [byte rdi + 64]
	MOVDQU xmm3, [byte rsi + 48]
	PSUBD xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQU xmm11, [byte rdi + 80]
	MOVDQU xmm6, [byte rsi + 64]
	MOVDQA [byte rdx + 112], xmm2
	MOVDQU xmm4, [byte rdi + 96]
	MOVDQU xmm7, [byte rsi + 80]
	PSUBD xmm0, xmm1
	ADD rdx, 128
	MOVDQU xmm2, [byte rdi + 112]
	MOVDQU xmm5, [byte rsi + 96]
	PSUBD xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 128
	MOVDQU xmm15, [byte rsi + 112]
	PSUBD xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	PSUBD xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBD xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBD xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBD xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBD xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32sV32s_V32s_SandyBridge
_yepCore_Subtract_V32sV32s_V32s_SandyBridge:
%else
section .text
global __yepCore_Subtract_V32sV32s_V32s_SandyBridge
__yepCore_Subtract_V32sV32s_V32s_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVDQU xmm0, [rdi]
	VMOVDQU xmm8, [byte rdi + 16]
	VMOVDQU xmm1, [rsi]
	VMOVDQU xmm9, [byte rdi + 32]
	VMOVDQU xmm13, [byte rsi + 16]
	VMOVDQU xmm12, [byte rdi + 48]
	VMOVDQU xmm10, [byte rsi + 32]
	VMOVDQU xmm14, [byte rdi + 64]
	VMOVDQU xmm3, [byte rsi + 48]
	VMOVDQU xmm11, [byte rdi + 80]
	VMOVDQU xmm6, [byte rsi + 64]
	VMOVDQU xmm4, [byte rdi + 96]
	VMOVDQU xmm7, [byte rsi + 80]
	VPSUBD xmm0, xmm0, xmm1
	VMOVDQU xmm2, [byte rdi + 112]
	VMOVDQU xmm5, [byte rsi + 96]
	VPSUBD xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 128
	VMOVDQU xmm15, [byte rsi + 112]
	VPSUBD xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVDQU xmm0, [rdi]
	ADD rsi, 128
	VPSUBD xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VMOVDQU xmm8, [byte rdi + 16]
	VMOVDQU xmm1, [rsi]
	VPSUBD xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VMOVDQU xmm9, [byte rdi + 32]
	VMOVDQU xmm13, [byte rsi + 16]
	VPSUBD xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VMOVDQU xmm12, [byte rdi + 48]
	VMOVDQU xmm10, [byte rsi + 32]
	VPSUBD xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VMOVDQU xmm14, [byte rdi + 64]
	VMOVDQU xmm3, [byte rsi + 48]
	VPSUBD xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQU xmm11, [byte rdi + 80]
	VMOVDQU xmm6, [byte rsi + 64]
	VMOVDQA [byte rdx + 112], xmm2
	VMOVDQU xmm4, [byte rdi + 96]
	VMOVDQU xmm7, [byte rsi + 80]
	VPSUBD xmm0, xmm0, xmm1
	ADD rdx, 128
	VMOVDQU xmm2, [byte rdi + 112]
	VMOVDQU xmm5, [byte rsi + 96]
	VPSUBD xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 128
	VMOVDQU xmm15, [byte rsi + 112]
	VPSUBD xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBD xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBD xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBD xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBD xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBD xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32sV32s_V32s_Haswell
_yepCore_Subtract_V32sV32s_V32s_Haswell:
%else
section .text
global __yepCore_Subtract_V32sV32s_V32s_Haswell
__yepCore_Subtract_V32sV32s_V32s_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 64
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVDQU ymm0, [rdi]
	VMOVDQU ymm8, [byte rdi + 32]
	VMOVDQU ymm7, [rsi]
	VMOVDQU ymm9, [byte rdi + 64]
	VMOVDQU ymm5, [byte rsi + 32]
	VMOVDQU ymm1, [byte rdi + 96]
	VMOVDQU ymm3, [byte rsi + 64]
	VMOVDQU ymm14, [dword rdi + 128]
	VMOVDQU ymm2, [byte rsi + 96]
	VMOVDQU ymm12, [dword rdi + 160]
	VMOVDQU ymm15, [dword rsi + 128]
	VMOVDQU ymm11, [dword rdi + 192]
	VMOVDQU ymm10, [dword rsi + 160]
	VPSUBD ymm0, ymm0, ymm7
	VMOVDQU ymm13, [dword rdi + 224]
	VMOVDQU ymm4, [dword rsi + 192]
	VPSUBD ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 256
	VMOVDQU ymm6, [dword rsi + 224]
	VPSUBD ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 64
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVDQU ymm0, [rdi]
	ADD rsi, 256
	VPSUBD ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VMOVDQU ymm8, [byte rdi + 32]
	VMOVDQU ymm7, [rsi]
	VPSUBD ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VMOVDQU ymm9, [byte rdi + 64]
	VMOVDQU ymm5, [byte rsi + 32]
	VPSUBD ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VMOVDQU ymm1, [byte rdi + 96]
	VMOVDQU ymm3, [byte rsi + 64]
	VPSUBD ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VMOVDQU ymm14, [dword rdi + 128]
	VMOVDQU ymm2, [byte rsi + 96]
	VPSUBD ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQU ymm12, [dword rdi + 160]
	VMOVDQU ymm15, [dword rsi + 128]
	VMOVDQA [dword rdx + 224], ymm13
	VMOVDQU ymm11, [dword rdi + 192]
	VMOVDQU ymm10, [dword rsi + 160]
	VPSUBD ymm0, ymm0, ymm7
	ADD rdx, 256
	VMOVDQU ymm13, [dword rdi + 224]
	VMOVDQU ymm4, [dword rsi + 192]
	VPSUBD ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 256
	VMOVDQU ymm6, [dword rsi + 224]
	VPSUBD ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 64
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 256
	VPSUBD ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBD ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBD ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBD ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBD ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 64
	JZ .return_ok
	.process_single:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB eax, r10d
	MOV [rdx], eax
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.K10 progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32sV32s_V64s_K10
_yepCore_Subtract_V32sV32s_V64s_K10:
%else
section .text
global __yepCore_Subtract_V32sV32s_V64s_K10
__yepCore_Subtract_V32sV32s_V64s_K10:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX rax, dword [rdi]
	ADD rdi, 4
	MOVSX r10, dword [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 8
	JB .batch_process_finish
	.process_batch_prologue:
	MOVQ xmm3, [rdi]
	MOVQ xmm2, [rsi]
	PXOR xmm1, xmm1
	PXOR xmm0, xmm0
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PXOR xmm7, xmm7
	PXOR xmm11, xmm11
	PCMPGTD xmm1, xmm3
	PCMPGTD xmm0, xmm2
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PXOR xmm12, xmm12
	PXOR xmm6, xmm6
	PCMPGTD xmm7, xmm8
	PCMPGTD xmm11, xmm14
	PUNPCKLDQ xmm3, xmm1
	PUNPCKLDQ xmm2, xmm0
	PSUBQ xmm3, xmm2
	MOVQ xmm13, [byte rdi + 24]
	MOVQ xmm5, [byte rsi + 24]
	PXOR xmm4, xmm4
	PXOR xmm15, xmm15
	PCMPGTD xmm12, xmm9
	PCMPGTD xmm6, xmm10
	PUNPCKLDQ xmm8, xmm7
	PUNPCKLDQ xmm14, xmm11
	PSUBQ xmm8, xmm14
	MOVDQA [rdx], xmm3
	ADD rdi, 32
	ADD rsi, 32
	PCMPGTD xmm4, xmm13
	PCMPGTD xmm15, xmm5
	PUNPCKLDQ xmm9, xmm12
	PUNPCKLDQ xmm10, xmm6
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 8
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVQ xmm3, [rdi]
	MOVQ xmm2, [rsi]
	PXOR xmm1, xmm1
	PXOR xmm0, xmm0
	PUNPCKLDQ xmm13, xmm4
	PUNPCKLDQ xmm5, xmm15
	PSUBQ xmm13, xmm5
	MOVDQA [byte rdx + 32], xmm9
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PXOR xmm7, xmm7
	PXOR xmm11, xmm11
	PCMPGTD xmm1, xmm3
	PCMPGTD xmm0, xmm2
	MOVDQA [byte rdx + 48], xmm13
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PXOR xmm12, xmm12
	PXOR xmm6, xmm6
	PCMPGTD xmm7, xmm8
	PCMPGTD xmm11, xmm14
	PUNPCKLDQ xmm3, xmm1
	PUNPCKLDQ xmm2, xmm0
	PSUBQ xmm3, xmm2
	ADD rdx, 64
	MOVQ xmm13, [byte rdi + 24]
	MOVQ xmm5, [byte rsi + 24]
	PXOR xmm4, xmm4
	PXOR xmm15, xmm15
	PCMPGTD xmm12, xmm9
	PCMPGTD xmm6, xmm10
	PUNPCKLDQ xmm8, xmm7
	PUNPCKLDQ xmm14, xmm11
	PSUBQ xmm8, xmm14
	MOVDQA [rdx], xmm3
	ADD rdi, 32
	ADD rsi, 32
	PCMPGTD xmm4, xmm13
	PCMPGTD xmm15, xmm5
	PUNPCKLDQ xmm9, xmm12
	PUNPCKLDQ xmm10, xmm6
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 8
	JAE .process_batch
	.process_batch_epilogue:
	PUNPCKLDQ xmm13, xmm4
	PUNPCKLDQ xmm5, xmm15
	PSUBQ xmm13, xmm5
	MOVDQA [byte rdx + 32], xmm9
	MOVDQA [byte rdx + 48], xmm13
	ADD rdx, 64
	.batch_process_finish:
	ADD rcx, 8
	JZ .return_ok
	.process_single:
	MOVSX rax, dword [rdi]
	ADD rdi, 4
	MOVSX r10, dword [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32sV32s_V64s_Nehalem
_yepCore_Subtract_V32sV32s_V64s_Nehalem:
%else
section .text
global __yepCore_Subtract_V32sV32s_V64s_Nehalem
__yepCore_Subtract_V32sV32s_V64s_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX rax, dword [rdi]
	ADD rdi, 4
	MOVSX r10, dword [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 16
	JB .batch_process_finish
	.process_batch_prologue:
	PMOVSXDQ xmm0, [rdi]
	PMOVSXDQ xmm8, [byte rdi + 8]
	PMOVSXDQ xmm1, [rsi]
	PMOVSXDQ xmm9, [byte rdi + 16]
	PMOVSXDQ xmm13, [byte rsi + 8]
	PMOVSXDQ xmm12, [byte rdi + 24]
	PMOVSXDQ xmm10, [byte rsi + 16]
	PMOVSXDQ xmm14, [byte rdi + 32]
	PMOVSXDQ xmm3, [byte rsi + 24]
	PMOVSXDQ xmm11, [byte rdi + 40]
	PMOVSXDQ xmm6, [byte rsi + 32]
	PMOVSXDQ xmm4, [byte rdi + 48]
	PMOVSXDQ xmm7, [byte rsi + 40]
	PSUBQ xmm0, xmm1
	PMOVSXDQ xmm2, [byte rdi + 56]
	PMOVSXDQ xmm5, [byte rsi + 48]
	PSUBQ xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVSXDQ xmm15, [byte rsi + 56]
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JB .process_batch_epilogue
	align 16
	.process_batch:
	PMOVSXDQ xmm0, [rdi]
	ADD rsi, 64
	PSUBQ xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PMOVSXDQ xmm8, [byte rdi + 8]
	PMOVSXDQ xmm1, [rsi]
	PSUBQ xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PMOVSXDQ xmm9, [byte rdi + 16]
	PMOVSXDQ xmm13, [byte rsi + 8]
	PSUBQ xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PMOVSXDQ xmm12, [byte rdi + 24]
	PMOVSXDQ xmm10, [byte rsi + 16]
	PSUBQ xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PMOVSXDQ xmm14, [byte rdi + 32]
	PMOVSXDQ xmm3, [byte rsi + 24]
	PSUBQ xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	PMOVSXDQ xmm11, [byte rdi + 40]
	PMOVSXDQ xmm6, [byte rsi + 32]
	MOVDQA [byte rdx + 112], xmm2
	PMOVSXDQ xmm4, [byte rdi + 48]
	PMOVSXDQ xmm7, [byte rsi + 40]
	PSUBQ xmm0, xmm1
	ADD rdx, 128
	PMOVSXDQ xmm2, [byte rdi + 56]
	PMOVSXDQ xmm5, [byte rsi + 48]
	PSUBQ xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVSXDQ xmm15, [byte rsi + 56]
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	PSUBQ xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBQ xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBQ xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBQ xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBQ xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 16
	JZ .return_ok
	.process_single:
	MOVSX rax, dword [rdi]
	ADD rdi, 4
	MOVSX r10, dword [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32sV32s_V64s_SandyBridge
_yepCore_Subtract_V32sV32s_V64s_SandyBridge:
%else
section .text
global __yepCore_Subtract_V32sV32s_V64s_SandyBridge
__yepCore_Subtract_V32sV32s_V64s_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSX rax, dword [rdi]
	ADD rdi, 4
	MOVSX r10, dword [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 16
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVSXDQ xmm0, [rdi]
	VPMOVSXDQ xmm8, [byte rdi + 8]
	VPMOVSXDQ xmm1, [rsi]
	VPMOVSXDQ xmm9, [byte rdi + 16]
	VPMOVSXDQ xmm13, [byte rsi + 8]
	VPMOVSXDQ xmm12, [byte rdi + 24]
	VPMOVSXDQ xmm10, [byte rsi + 16]
	VPMOVSXDQ xmm14, [byte rdi + 32]
	VPMOVSXDQ xmm3, [byte rsi + 24]
	VPMOVSXDQ xmm11, [byte rdi + 40]
	VPMOVSXDQ xmm6, [byte rsi + 32]
	VPMOVSXDQ xmm4, [byte rdi + 48]
	VPMOVSXDQ xmm7, [byte rsi + 40]
	VPSUBQ xmm0, xmm0, xmm1
	VPMOVSXDQ xmm2, [byte rdi + 56]
	VPMOVSXDQ xmm5, [byte rsi + 48]
	VPSUBQ xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVSXDQ xmm15, [byte rsi + 56]
	VPSUBQ xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVSXDQ xmm0, [rdi]
	ADD rsi, 64
	VPSUBQ xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPMOVSXDQ xmm8, [byte rdi + 8]
	VPMOVSXDQ xmm1, [rsi]
	VPSUBQ xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPMOVSXDQ xmm9, [byte rdi + 16]
	VPMOVSXDQ xmm13, [byte rsi + 8]
	VPSUBQ xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPMOVSXDQ xmm12, [byte rdi + 24]
	VPMOVSXDQ xmm10, [byte rsi + 16]
	VPSUBQ xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPMOVSXDQ xmm14, [byte rdi + 32]
	VPMOVSXDQ xmm3, [byte rsi + 24]
	VPSUBQ xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VPMOVSXDQ xmm11, [byte rdi + 40]
	VPMOVSXDQ xmm6, [byte rsi + 32]
	VMOVDQA [byte rdx + 112], xmm2
	VPMOVSXDQ xmm4, [byte rdi + 48]
	VPMOVSXDQ xmm7, [byte rsi + 40]
	VPSUBQ xmm0, xmm0, xmm1
	ADD rdx, 128
	VPMOVSXDQ xmm2, [byte rdi + 56]
	VPMOVSXDQ xmm5, [byte rsi + 48]
	VPSUBQ xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVSXDQ xmm15, [byte rsi + 56]
	VPSUBQ xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	VPSUBQ xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBQ xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBQ xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBQ xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBQ xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 16
	JZ .return_ok
	.process_single:
	MOVSX rax, dword [rdi]
	ADD rdi, 4
	MOVSX r10, dword [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32sV32s_V64s_Haswell
_yepCore_Subtract_V32sV32s_V64s_Haswell:
%else
section .text
global __yepCore_Subtract_V32sV32s_V64s_Haswell
__yepCore_Subtract_V32sV32s_V64s_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOVSX rax, dword [rdi]
	ADD rdi, 4
	MOVSX r10, dword [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVSXDQ ymm0, [rdi]
	VPMOVSXDQ ymm8, [byte rdi + 16]
	VPMOVSXDQ ymm7, [rsi]
	VPMOVSXDQ ymm9, [byte rdi + 32]
	VPMOVSXDQ ymm5, [byte rsi + 16]
	VPMOVSXDQ ymm1, [byte rdi + 48]
	VPMOVSXDQ ymm3, [byte rsi + 32]
	VPMOVSXDQ ymm14, [byte rdi + 64]
	VPMOVSXDQ ymm2, [byte rsi + 48]
	VPMOVSXDQ ymm12, [byte rdi + 80]
	VPMOVSXDQ ymm15, [byte rsi + 64]
	VPMOVSXDQ ymm11, [byte rdi + 96]
	VPMOVSXDQ ymm10, [byte rsi + 80]
	VPSUBQ ymm0, ymm0, ymm7
	VPMOVSXDQ ymm13, [byte rdi + 112]
	VPMOVSXDQ ymm4, [byte rsi + 96]
	VPSUBQ ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVSXDQ ymm6, [byte rsi + 112]
	VPSUBQ ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVSXDQ ymm0, [rdi]
	ADD rsi, 128
	VPSUBQ ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPMOVSXDQ ymm8, [byte rdi + 16]
	VPMOVSXDQ ymm7, [rsi]
	VPSUBQ ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPMOVSXDQ ymm9, [byte rdi + 32]
	VPMOVSXDQ ymm5, [byte rsi + 16]
	VPSUBQ ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPMOVSXDQ ymm1, [byte rdi + 48]
	VPMOVSXDQ ymm3, [byte rsi + 32]
	VPSUBQ ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPMOVSXDQ ymm14, [byte rdi + 64]
	VPMOVSXDQ ymm2, [byte rsi + 48]
	VPSUBQ ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VPMOVSXDQ ymm12, [byte rdi + 80]
	VPMOVSXDQ ymm15, [byte rsi + 64]
	VMOVDQA [dword rdx + 224], ymm13
	VPMOVSXDQ ymm11, [byte rdi + 96]
	VPMOVSXDQ ymm10, [byte rsi + 80]
	VPSUBQ ymm0, ymm0, ymm7
	ADD rdx, 256
	VPMOVSXDQ ymm13, [byte rdi + 112]
	VPMOVSXDQ ymm4, [byte rsi + 96]
	VPSUBQ ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVSXDQ ymm6, [byte rsi + 112]
	VPSUBQ ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBQ ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBQ ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBQ ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBQ ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBQ ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOVSX rax, dword [rdi]
	ADD rdi, 4
	MOVSX r10, dword [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.K10 progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32uV32u_V64u_K10
_yepCore_Subtract_V32uV32u_V64u_K10:
%else
section .text
global __yepCore_Subtract_V32uV32u_V64u_K10
__yepCore_Subtract_V32uV32u_V64u_K10:
%endif
	.ENTRY:
	PXOR xmm3, xmm3
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 14
	JB .batch_process_finish
	.process_batch_prologue:
	MOVQ xmm2, [rdi]
	MOVQ xmm1, [rsi]
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	MOVQ xmm15, [byte rdi + 24]
	MOVQ xmm13, [byte rsi + 24]
	PUNPCKLDQ xmm2, xmm3
	MOVQ xmm7, [byte rdi + 32]
	MOVQ xmm4, [byte rsi + 32]
	PUNPCKLDQ xmm8, xmm3
	PUNPCKLDQ xmm1, xmm3
	MOVQ xmm12, [byte rdi + 40]
	MOVQ xmm11, [byte rsi + 40]
	PUNPCKLDQ xmm9, xmm3
	PUNPCKLDQ xmm14, xmm3
	PSUBQ xmm2, xmm1
	MOVQ xmm5, [byte rdi + 48]
	MOVQ xmm6, [byte rsi + 48]
	PUNPCKLDQ xmm15, xmm3
	PUNPCKLDQ xmm10, xmm3
	PSUBQ xmm8, xmm14
	MOVDQA [rdx], xmm2
	ADD rdi, 56
	ADD rsi, 56
	PUNPCKLDQ xmm7, xmm3
	PUNPCKLDQ xmm13, xmm3
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 14
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVQ xmm2, [rdi]
	MOVQ xmm1, [rsi]
	PUNPCKLDQ xmm12, xmm3
	PUNPCKLDQ xmm4, xmm3
	PSUBQ xmm15, xmm13
	MOVDQA [byte rdx + 32], xmm9
	MOVQ xmm8, [byte rdi + 8]
	MOVQ xmm14, [byte rsi + 8]
	PUNPCKLDQ xmm5, xmm3
	PUNPCKLDQ xmm11, xmm3
	PSUBQ xmm7, xmm4
	MOVDQA [byte rdx + 48], xmm15
	MOVQ xmm9, [byte rdi + 16]
	MOVQ xmm10, [byte rsi + 16]
	PUNPCKLDQ xmm6, xmm3
	PSUBQ xmm12, xmm11
	MOVDQA [byte rdx + 64], xmm7
	MOVQ xmm15, [byte rdi + 24]
	MOVQ xmm13, [byte rsi + 24]
	PUNPCKLDQ xmm2, xmm3
	PSUBQ xmm5, xmm6
	MOVDQA [byte rdx + 80], xmm12
	MOVQ xmm7, [byte rdi + 32]
	MOVQ xmm4, [byte rsi + 32]
	PUNPCKLDQ xmm8, xmm3
	PUNPCKLDQ xmm1, xmm3
	MOVDQA [byte rdx + 96], xmm5
	MOVQ xmm12, [byte rdi + 40]
	MOVQ xmm11, [byte rsi + 40]
	PUNPCKLDQ xmm9, xmm3
	PUNPCKLDQ xmm14, xmm3
	PSUBQ xmm2, xmm1
	ADD rdx, 112
	MOVQ xmm5, [byte rdi + 48]
	MOVQ xmm6, [byte rsi + 48]
	PUNPCKLDQ xmm15, xmm3
	PUNPCKLDQ xmm10, xmm3
	PSUBQ xmm8, xmm14
	MOVDQA [rdx], xmm2
	ADD rdi, 56
	ADD rsi, 56
	PUNPCKLDQ xmm7, xmm3
	PUNPCKLDQ xmm13, xmm3
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 14
	JAE .process_batch
	.process_batch_epilogue:
	PUNPCKLDQ xmm12, xmm3
	PUNPCKLDQ xmm4, xmm3
	PSUBQ xmm15, xmm13
	MOVDQA [byte rdx + 32], xmm9
	PUNPCKLDQ xmm5, xmm3
	PUNPCKLDQ xmm11, xmm3
	PSUBQ xmm7, xmm4
	MOVDQA [byte rdx + 48], xmm15
	PUNPCKLDQ xmm6, xmm3
	PSUBQ xmm12, xmm11
	MOVDQA [byte rdx + 64], xmm7
	PSUBQ xmm5, xmm6
	MOVDQA [byte rdx + 80], xmm12
	MOVDQA [byte rdx + 96], xmm5
	ADD rdx, 112
	.batch_process_finish:
	ADD rcx, 14
	JZ .return_ok
	.process_single:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32uV32u_V64u_Nehalem
_yepCore_Subtract_V32uV32u_V64u_Nehalem:
%else
section .text
global __yepCore_Subtract_V32uV32u_V64u_Nehalem
__yepCore_Subtract_V32uV32u_V64u_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 16
	JB .batch_process_finish
	.process_batch_prologue:
	PMOVZXDQ xmm0, [rdi]
	PMOVZXDQ xmm8, [byte rdi + 8]
	PMOVZXDQ xmm1, [rsi]
	PMOVZXDQ xmm9, [byte rdi + 16]
	PMOVZXDQ xmm13, [byte rsi + 8]
	PMOVZXDQ xmm12, [byte rdi + 24]
	PMOVZXDQ xmm10, [byte rsi + 16]
	PMOVZXDQ xmm14, [byte rdi + 32]
	PMOVZXDQ xmm3, [byte rsi + 24]
	PMOVZXDQ xmm11, [byte rdi + 40]
	PMOVZXDQ xmm6, [byte rsi + 32]
	PMOVZXDQ xmm4, [byte rdi + 48]
	PMOVZXDQ xmm7, [byte rsi + 40]
	PSUBQ xmm0, xmm1
	PMOVZXDQ xmm2, [byte rdi + 56]
	PMOVZXDQ xmm5, [byte rsi + 48]
	PSUBQ xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVZXDQ xmm15, [byte rsi + 56]
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JB .process_batch_epilogue
	align 16
	.process_batch:
	PMOVZXDQ xmm0, [rdi]
	ADD rsi, 64
	PSUBQ xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PMOVZXDQ xmm8, [byte rdi + 8]
	PMOVZXDQ xmm1, [rsi]
	PSUBQ xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PMOVZXDQ xmm9, [byte rdi + 16]
	PMOVZXDQ xmm13, [byte rsi + 8]
	PSUBQ xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PMOVZXDQ xmm12, [byte rdi + 24]
	PMOVZXDQ xmm10, [byte rsi + 16]
	PSUBQ xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PMOVZXDQ xmm14, [byte rdi + 32]
	PMOVZXDQ xmm3, [byte rsi + 24]
	PSUBQ xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	PMOVZXDQ xmm11, [byte rdi + 40]
	PMOVZXDQ xmm6, [byte rsi + 32]
	MOVDQA [byte rdx + 112], xmm2
	PMOVZXDQ xmm4, [byte rdi + 48]
	PMOVZXDQ xmm7, [byte rsi + 40]
	PSUBQ xmm0, xmm1
	ADD rdx, 128
	PMOVZXDQ xmm2, [byte rdi + 56]
	PMOVZXDQ xmm5, [byte rsi + 48]
	PSUBQ xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 64
	PMOVZXDQ xmm15, [byte rsi + 56]
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	PSUBQ xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBQ xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBQ xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBQ xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBQ xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 16
	JZ .return_ok
	.process_single:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32uV32u_V64u_SandyBridge
_yepCore_Subtract_V32uV32u_V64u_SandyBridge:
%else
section .text
global __yepCore_Subtract_V32uV32u_V64u_SandyBridge
__yepCore_Subtract_V32uV32u_V64u_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 16
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVZXDQ xmm0, [rdi]
	VPMOVZXDQ xmm8, [byte rdi + 8]
	VPMOVZXDQ xmm1, [rsi]
	VPMOVZXDQ xmm9, [byte rdi + 16]
	VPMOVZXDQ xmm13, [byte rsi + 8]
	VPMOVZXDQ xmm12, [byte rdi + 24]
	VPMOVZXDQ xmm10, [byte rsi + 16]
	VPMOVZXDQ xmm14, [byte rdi + 32]
	VPMOVZXDQ xmm3, [byte rsi + 24]
	VPMOVZXDQ xmm11, [byte rdi + 40]
	VPMOVZXDQ xmm6, [byte rsi + 32]
	VPMOVZXDQ xmm4, [byte rdi + 48]
	VPMOVZXDQ xmm7, [byte rsi + 40]
	VPSUBQ xmm0, xmm0, xmm1
	VPMOVZXDQ xmm2, [byte rdi + 56]
	VPMOVZXDQ xmm5, [byte rsi + 48]
	VPSUBQ xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVZXDQ xmm15, [byte rsi + 56]
	VPSUBQ xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVZXDQ xmm0, [rdi]
	ADD rsi, 64
	VPSUBQ xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPMOVZXDQ xmm8, [byte rdi + 8]
	VPMOVZXDQ xmm1, [rsi]
	VPSUBQ xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPMOVZXDQ xmm9, [byte rdi + 16]
	VPMOVZXDQ xmm13, [byte rsi + 8]
	VPSUBQ xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPMOVZXDQ xmm12, [byte rdi + 24]
	VPMOVZXDQ xmm10, [byte rsi + 16]
	VPSUBQ xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPMOVZXDQ xmm14, [byte rdi + 32]
	VPMOVZXDQ xmm3, [byte rsi + 24]
	VPSUBQ xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VPMOVZXDQ xmm11, [byte rdi + 40]
	VPMOVZXDQ xmm6, [byte rsi + 32]
	VMOVDQA [byte rdx + 112], xmm2
	VPMOVZXDQ xmm4, [byte rdi + 48]
	VPMOVZXDQ xmm7, [byte rsi + 40]
	VPSUBQ xmm0, xmm0, xmm1
	ADD rdx, 128
	VPMOVZXDQ xmm2, [byte rdi + 56]
	VPMOVZXDQ xmm5, [byte rsi + 48]
	VPSUBQ xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 64
	VPMOVZXDQ xmm15, [byte rsi + 56]
	VPSUBQ xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 64
	VPSUBQ xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBQ xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBQ xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBQ xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBQ xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 16
	JZ .return_ok
	.process_single:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32uV32u_V64u_Haswell
_yepCore_Subtract_V32uV32u_V64u_Haswell:
%else
section .text
global __yepCore_Subtract_V32uV32u_V64u_Haswell
__yepCore_Subtract_V32uV32u_V64u_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	VPMOVZXDQ ymm0, [rdi]
	VPMOVZXDQ ymm8, [byte rdi + 16]
	VPMOVZXDQ ymm7, [rsi]
	VPMOVZXDQ ymm9, [byte rdi + 32]
	VPMOVZXDQ ymm5, [byte rsi + 16]
	VPMOVZXDQ ymm1, [byte rdi + 48]
	VPMOVZXDQ ymm3, [byte rsi + 32]
	VPMOVZXDQ ymm14, [byte rdi + 64]
	VPMOVZXDQ ymm2, [byte rsi + 48]
	VPMOVZXDQ ymm12, [byte rdi + 80]
	VPMOVZXDQ ymm15, [byte rsi + 64]
	VPMOVZXDQ ymm11, [byte rdi + 96]
	VPMOVZXDQ ymm10, [byte rsi + 80]
	VPSUBQ ymm0, ymm0, ymm7
	VPMOVZXDQ ymm13, [byte rdi + 112]
	VPMOVZXDQ ymm4, [byte rsi + 96]
	VPSUBQ ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVZXDQ ymm6, [byte rsi + 112]
	VPSUBQ ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VPMOVZXDQ ymm0, [rdi]
	ADD rsi, 128
	VPSUBQ ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPMOVZXDQ ymm8, [byte rdi + 16]
	VPMOVZXDQ ymm7, [rsi]
	VPSUBQ ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPMOVZXDQ ymm9, [byte rdi + 32]
	VPMOVZXDQ ymm5, [byte rsi + 16]
	VPSUBQ ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPMOVZXDQ ymm1, [byte rdi + 48]
	VPMOVZXDQ ymm3, [byte rsi + 32]
	VPSUBQ ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPMOVZXDQ ymm14, [byte rdi + 64]
	VPMOVZXDQ ymm2, [byte rsi + 48]
	VPSUBQ ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VPMOVZXDQ ymm12, [byte rdi + 80]
	VPMOVZXDQ ymm15, [byte rsi + 64]
	VMOVDQA [dword rdx + 224], ymm13
	VPMOVZXDQ ymm11, [byte rdi + 96]
	VPMOVZXDQ ymm10, [byte rsi + 80]
	VPSUBQ ymm0, ymm0, ymm7
	ADD rdx, 256
	VPMOVZXDQ ymm13, [byte rdi + 112]
	VPMOVZXDQ ymm4, [byte rsi + 96]
	VPSUBQ ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 128
	VPMOVZXDQ ymm6, [byte rsi + 112]
	VPSUBQ ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBQ ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBQ ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBQ ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBQ ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBQ ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOV eax, [rdi]
	ADD rdi, 4
	MOV r10d, [rsi]
	ADD rsi, 4
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V64sV64s_V64s_Nehalem
_yepCore_Subtract_V64sV64s_V64s_Nehalem:
%else
section .text
global __yepCore_Subtract_V64sV64s_V64s_Nehalem
__yepCore_Subtract_V64sV64s_V64s_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 7
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 7
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOV rax, [rdi]
	ADD rdi, 8
	MOV r10, [rsi]
	ADD rsi, 8
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 16
	JB .batch_process_finish
	.process_batch_prologue:
	MOVDQU xmm0, [rdi]
	MOVDQU xmm8, [byte rdi + 16]
	MOVDQU xmm1, [rsi]
	MOVDQU xmm9, [byte rdi + 32]
	MOVDQU xmm13, [byte rsi + 16]
	MOVDQU xmm12, [byte rdi + 48]
	MOVDQU xmm10, [byte rsi + 32]
	MOVDQU xmm14, [byte rdi + 64]
	MOVDQU xmm3, [byte rsi + 48]
	MOVDQU xmm11, [byte rdi + 80]
	MOVDQU xmm6, [byte rsi + 64]
	MOVDQU xmm4, [byte rdi + 96]
	MOVDQU xmm7, [byte rsi + 80]
	PSUBQ xmm0, xmm1
	MOVDQU xmm2, [byte rdi + 112]
	MOVDQU xmm5, [byte rsi + 96]
	PSUBQ xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 128
	MOVDQU xmm15, [byte rsi + 112]
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVDQU xmm0, [rdi]
	ADD rsi, 128
	PSUBQ xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	MOVDQU xmm8, [byte rdi + 16]
	MOVDQU xmm1, [rsi]
	PSUBQ xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	MOVDQU xmm9, [byte rdi + 32]
	MOVDQU xmm13, [byte rsi + 16]
	PSUBQ xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	MOVDQU xmm12, [byte rdi + 48]
	MOVDQU xmm10, [byte rsi + 32]
	PSUBQ xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	MOVDQU xmm14, [byte rdi + 64]
	MOVDQU xmm3, [byte rsi + 48]
	PSUBQ xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQU xmm11, [byte rdi + 80]
	MOVDQU xmm6, [byte rsi + 64]
	MOVDQA [byte rdx + 112], xmm2
	MOVDQU xmm4, [byte rdi + 96]
	MOVDQU xmm7, [byte rsi + 80]
	PSUBQ xmm0, xmm1
	ADD rdx, 128
	MOVDQU xmm2, [byte rdi + 112]
	MOVDQU xmm5, [byte rsi + 96]
	PSUBQ xmm8, xmm13
	MOVDQA [rdx], xmm0
	ADD rdi, 128
	MOVDQU xmm15, [byte rsi + 112]
	PSUBQ xmm9, xmm10
	MOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	PSUBQ xmm12, xmm3
	MOVDQA [byte rdx + 32], xmm9
	PSUBQ xmm14, xmm6
	MOVDQA [byte rdx + 48], xmm12
	PSUBQ xmm11, xmm7
	MOVDQA [byte rdx + 64], xmm14
	PSUBQ xmm4, xmm5
	MOVDQA [byte rdx + 80], xmm11
	PSUBQ xmm2, xmm15
	MOVDQA [byte rdx + 96], xmm4
	MOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 16
	JZ .return_ok
	.process_single:
	MOV rax, [rdi]
	ADD rdi, 8
	MOV r10, [rsi]
	ADD rsi, 8
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V64sV64s_V64s_SandyBridge
_yepCore_Subtract_V64sV64s_V64s_SandyBridge:
%else
section .text
global __yepCore_Subtract_V64sV64s_V64s_SandyBridge
__yepCore_Subtract_V64sV64s_V64s_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 7
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 7
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOV rax, [rdi]
	ADD rdi, 8
	MOV r10, [rsi]
	ADD rsi, 8
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 16
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVDQU xmm0, [rdi]
	VMOVDQU xmm8, [byte rdi + 16]
	VMOVDQU xmm1, [rsi]
	VMOVDQU xmm9, [byte rdi + 32]
	VMOVDQU xmm13, [byte rsi + 16]
	VMOVDQU xmm12, [byte rdi + 48]
	VMOVDQU xmm10, [byte rsi + 32]
	VMOVDQU xmm14, [byte rdi + 64]
	VMOVDQU xmm3, [byte rsi + 48]
	VMOVDQU xmm11, [byte rdi + 80]
	VMOVDQU xmm6, [byte rsi + 64]
	VMOVDQU xmm4, [byte rdi + 96]
	VMOVDQU xmm7, [byte rsi + 80]
	VPSUBQ xmm0, xmm0, xmm1
	VMOVDQU xmm2, [byte rdi + 112]
	VMOVDQU xmm5, [byte rsi + 96]
	VPSUBQ xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 128
	VMOVDQU xmm15, [byte rsi + 112]
	VPSUBQ xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVDQU xmm0, [rdi]
	ADD rsi, 128
	VPSUBQ xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VMOVDQU xmm8, [byte rdi + 16]
	VMOVDQU xmm1, [rsi]
	VPSUBQ xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VMOVDQU xmm9, [byte rdi + 32]
	VMOVDQU xmm13, [byte rsi + 16]
	VPSUBQ xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VMOVDQU xmm12, [byte rdi + 48]
	VMOVDQU xmm10, [byte rsi + 32]
	VPSUBQ xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VMOVDQU xmm14, [byte rdi + 64]
	VMOVDQU xmm3, [byte rsi + 48]
	VPSUBQ xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQU xmm11, [byte rdi + 80]
	VMOVDQU xmm6, [byte rsi + 64]
	VMOVDQA [byte rdx + 112], xmm2
	VMOVDQU xmm4, [byte rdi + 96]
	VMOVDQU xmm7, [byte rsi + 80]
	VPSUBQ xmm0, xmm0, xmm1
	ADD rdx, 128
	VMOVDQU xmm2, [byte rdi + 112]
	VMOVDQU xmm5, [byte rsi + 96]
	VPSUBQ xmm8, xmm8, xmm13
	VMOVDQA [rdx], xmm0
	ADD rdi, 128
	VMOVDQU xmm15, [byte rsi + 112]
	VPSUBQ xmm9, xmm9, xmm10
	VMOVDQA [byte rdx + 16], xmm8
	SUB rcx, 16
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 128
	VPSUBQ xmm12, xmm12, xmm3
	VMOVDQA [byte rdx + 32], xmm9
	VPSUBQ xmm14, xmm14, xmm6
	VMOVDQA [byte rdx + 48], xmm12
	VPSUBQ xmm11, xmm11, xmm7
	VMOVDQA [byte rdx + 64], xmm14
	VPSUBQ xmm4, xmm4, xmm5
	VMOVDQA [byte rdx + 80], xmm11
	VPSUBQ xmm2, xmm2, xmm15
	VMOVDQA [byte rdx + 96], xmm4
	VMOVDQA [byte rdx + 112], xmm2
	ADD rdx, 128
	.batch_process_finish:
	ADD rcx, 16
	JZ .return_ok
	.process_single:
	MOV rax, [rdi]
	ADD rdi, 8
	MOV r10, [rsi]
	ADD rsi, 8
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Haswell progbits alloc exec nowrite align=16
global _yepCore_Subtract_V64sV64s_V64s_Haswell
_yepCore_Subtract_V64sV64s_V64s_Haswell:
%else
section .text
global __yepCore_Subtract_V64sV64s_V64s_Haswell
__yepCore_Subtract_V64sV64s_V64s_Haswell:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 7
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 7
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	MOV rax, [rdi]
	ADD rdi, 8
	MOV r10, [rsi]
	ADD rsi, 8
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 32
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVDQU ymm0, [rdi]
	VMOVDQU ymm8, [byte rdi + 32]
	VMOVDQU ymm7, [rsi]
	VMOVDQU ymm9, [byte rdi + 64]
	VMOVDQU ymm5, [byte rsi + 32]
	VMOVDQU ymm1, [byte rdi + 96]
	VMOVDQU ymm3, [byte rsi + 64]
	VMOVDQU ymm14, [dword rdi + 128]
	VMOVDQU ymm2, [byte rsi + 96]
	VMOVDQU ymm12, [dword rdi + 160]
	VMOVDQU ymm15, [dword rsi + 128]
	VMOVDQU ymm11, [dword rdi + 192]
	VMOVDQU ymm10, [dword rsi + 160]
	VPSUBQ ymm0, ymm0, ymm7
	VMOVDQU ymm13, [dword rdi + 224]
	VMOVDQU ymm4, [dword rsi + 192]
	VPSUBQ ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 256
	VMOVDQU ymm6, [dword rsi + 224]
	VPSUBQ ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 32
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVDQU ymm0, [rdi]
	ADD rsi, 256
	VPSUBQ ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VMOVDQU ymm8, [byte rdi + 32]
	VMOVDQU ymm7, [rsi]
	VPSUBQ ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VMOVDQU ymm9, [byte rdi + 64]
	VMOVDQU ymm5, [byte rsi + 32]
	VPSUBQ ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VMOVDQU ymm1, [byte rdi + 96]
	VMOVDQU ymm3, [byte rsi + 64]
	VPSUBQ ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VMOVDQU ymm14, [dword rdi + 128]
	VMOVDQU ymm2, [byte rsi + 96]
	VPSUBQ ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQU ymm12, [dword rdi + 160]
	VMOVDQU ymm15, [dword rsi + 128]
	VMOVDQA [dword rdx + 224], ymm13
	VMOVDQU ymm11, [dword rdi + 192]
	VMOVDQU ymm10, [dword rsi + 160]
	VPSUBQ ymm0, ymm0, ymm7
	ADD rdx, 256
	VMOVDQU ymm13, [dword rdi + 224]
	VMOVDQU ymm4, [dword rsi + 192]
	VPSUBQ ymm8, ymm8, ymm5
	VMOVDQA [rdx], ymm0
	ADD rdi, 256
	VMOVDQU ymm6, [dword rsi + 224]
	VPSUBQ ymm9, ymm9, ymm3
	VMOVDQA [byte rdx + 32], ymm8
	SUB rcx, 32
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 256
	VPSUBQ ymm1, ymm1, ymm2
	VMOVDQA [byte rdx + 64], ymm9
	VPSUBQ ymm14, ymm14, ymm15
	VMOVDQA [byte rdx + 96], ymm1
	VPSUBQ ymm12, ymm12, ymm10
	VMOVDQA [dword rdx + 128], ymm14
	VPSUBQ ymm11, ymm11, ymm4
	VMOVDQA [dword rdx + 160], ymm12
	VPSUBQ ymm13, ymm13, ymm6
	VMOVDQA [dword rdx + 192], ymm11
	VMOVDQA [dword rdx + 224], ymm13
	ADD rdx, 256
	.batch_process_finish:
	ADD rcx, 32
	JZ .return_ok
	.process_single:
	MOV rax, [rdi]
	ADD rdi, 8
	MOV r10, [rsi]
	ADD rsi, 8
	SUB rax, r10
	MOV [rdx], rax
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32fV32f_V32f_Nehalem
_yepCore_Subtract_V32fV32f_V32f_Nehalem:
%else
section .text
global __yepCore_Subtract_V32fV32f_V32f_Nehalem
__yepCore_Subtract_V32fV32f_V32f_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSS xmm2, [rdi]
	ADD rdi, 4
	MOVSS xmm1, [rsi]
	ADD rsi, 4
	SUBSS xmm2, xmm1
	MOVSS [rdx], xmm2
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 28
	JB .batch_process_finish
	.process_batch_prologue:
	MOVUPS xmm2, [rdi]
	MOVUPS xmm8, [byte rdi + 16]
	MOVUPS xmm4, [rsi]
	MOVUPS xmm9, [byte rdi + 32]
	MOVUPS xmm3, [byte rsi + 16]
	MOVUPS xmm12, [byte rdi + 48]
	MOVUPS xmm13, [byte rsi + 32]
	MOVUPS xmm14, [byte rdi + 64]
	MOVUPS xmm10, [byte rsi + 48]
	MOVUPS xmm11, [byte rdi + 80]
	MOVUPS xmm5, [byte rsi + 64]
	SUBPS xmm2, xmm4
	MOVUPS xmm6, [byte rdi + 96]
	MOVUPS xmm7, [byte rsi + 80]
	SUBPS xmm8, xmm3
	MOVAPS [rdx], xmm2
	ADD rdi, 112
	MOVUPS xmm15, [byte rsi + 96]
	SUBPS xmm9, xmm13
	MOVAPS [byte rdx + 16], xmm8
	SUB rcx, 28
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVUPS xmm2, [rdi]
	ADD rsi, 112
	SUBPS xmm12, xmm10
	MOVAPS [byte rdx + 32], xmm9
	MOVUPS xmm8, [byte rdi + 16]
	MOVUPS xmm4, [rsi]
	SUBPS xmm14, xmm5
	MOVAPS [byte rdx + 48], xmm12
	MOVUPS xmm9, [byte rdi + 32]
	MOVUPS xmm3, [byte rsi + 16]
	SUBPS xmm11, xmm7
	MOVAPS [byte rdx + 64], xmm14
	MOVUPS xmm12, [byte rdi + 48]
	MOVUPS xmm13, [byte rsi + 32]
	SUBPS xmm6, xmm15
	MOVAPS [byte rdx + 80], xmm11
	MOVUPS xmm14, [byte rdi + 64]
	MOVUPS xmm10, [byte rsi + 48]
	MOVAPS [byte rdx + 96], xmm6
	MOVUPS xmm11, [byte rdi + 80]
	MOVUPS xmm5, [byte rsi + 64]
	SUBPS xmm2, xmm4
	ADD rdx, 112
	MOVUPS xmm6, [byte rdi + 96]
	MOVUPS xmm7, [byte rsi + 80]
	SUBPS xmm8, xmm3
	MOVAPS [rdx], xmm2
	ADD rdi, 112
	MOVUPS xmm15, [byte rsi + 96]
	SUBPS xmm9, xmm13
	MOVAPS [byte rdx + 16], xmm8
	SUB rcx, 28
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 112
	SUBPS xmm12, xmm10
	MOVAPS [byte rdx + 32], xmm9
	SUBPS xmm14, xmm5
	MOVAPS [byte rdx + 48], xmm12
	SUBPS xmm11, xmm7
	MOVAPS [byte rdx + 64], xmm14
	SUBPS xmm6, xmm15
	MOVAPS [byte rdx + 80], xmm11
	MOVAPS [byte rdx + 96], xmm6
	ADD rdx, 112
	.batch_process_finish:
	ADD rcx, 28
	JZ .return_ok
	.process_single:
	MOVSS xmm8, [rdi]
	ADD rdi, 4
	MOVSS xmm9, [rsi]
	ADD rsi, 4
	SUBSS xmm8, xmm9
	MOVSS [rdx], xmm8
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V32fV32f_V32f_SandyBridge
_yepCore_Subtract_V32fV32f_V32f_SandyBridge:
%else
section .text
global __yepCore_Subtract_V32fV32f_V32f_SandyBridge
__yepCore_Subtract_V32fV32f_V32f_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 3
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 3
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 3
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	VMOVSS xmm2, [rdi]
	ADD rdi, 4
	VMOVSS xmm1, [rsi]
	ADD rsi, 4
	VSUBSS xmm2, xmm2, xmm1
	VMOVSS [rdx], xmm2
	ADD rdx, 4
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 56
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVUPS ymm2, [rdi]
	VMOVUPS ymm8, [byte rdi + 32]
	VMOVUPS ymm13, [rsi]
	VMOVUPS ymm9, [byte rdi + 64]
	VMOVUPS ymm7, [byte rsi + 32]
	VMOVUPS ymm3, [byte rdi + 96]
	VMOVUPS ymm6, [byte rsi + 64]
	VMOVUPS ymm14, [dword rdi + 128]
	VMOVUPS ymm5, [byte rsi + 96]
	VMOVUPS ymm12, [dword rdi + 160]
	VMOVUPS ymm4, [dword rsi + 128]
	VSUBPS ymm2, ymm2, ymm13
	VMOVUPS ymm11, [dword rdi + 192]
	VMOVUPS ymm15, [dword rsi + 160]
	VSUBPS ymm8, ymm8, ymm7
	VMOVAPS [rdx], ymm2
	ADD rdi, 224
	VMOVUPS ymm10, [dword rsi + 192]
	VSUBPS ymm9, ymm9, ymm6
	VMOVAPS [byte rdx + 32], ymm8
	SUB rcx, 56
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVUPS ymm2, [rdi]
	ADD rsi, 224
	VSUBPS ymm3, ymm3, ymm5
	VMOVAPS [byte rdx + 64], ymm9
	VMOVUPS ymm8, [byte rdi + 32]
	VMOVUPS ymm13, [rsi]
	VSUBPS ymm14, ymm14, ymm4
	VMOVAPS [byte rdx + 96], ymm3
	VMOVUPS ymm9, [byte rdi + 64]
	VMOVUPS ymm7, [byte rsi + 32]
	VSUBPS ymm12, ymm12, ymm15
	VMOVAPS [dword rdx + 128], ymm14
	VMOVUPS ymm3, [byte rdi + 96]
	VMOVUPS ymm6, [byte rsi + 64]
	VSUBPS ymm11, ymm11, ymm10
	VMOVAPS [dword rdx + 160], ymm12
	VMOVUPS ymm14, [dword rdi + 128]
	VMOVUPS ymm5, [byte rsi + 96]
	VMOVAPS [dword rdx + 192], ymm11
	VMOVUPS ymm12, [dword rdi + 160]
	VMOVUPS ymm4, [dword rsi + 128]
	VSUBPS ymm2, ymm2, ymm13
	ADD rdx, 224
	VMOVUPS ymm11, [dword rdi + 192]
	VMOVUPS ymm15, [dword rsi + 160]
	VSUBPS ymm8, ymm8, ymm7
	VMOVAPS [rdx], ymm2
	ADD rdi, 224
	VMOVUPS ymm10, [dword rsi + 192]
	VSUBPS ymm9, ymm9, ymm6
	VMOVAPS [byte rdx + 32], ymm8
	SUB rcx, 56
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 224
	VSUBPS ymm3, ymm3, ymm5
	VMOVAPS [byte rdx + 64], ymm9
	VSUBPS ymm14, ymm14, ymm4
	VMOVAPS [byte rdx + 96], ymm3
	VSUBPS ymm12, ymm12, ymm15
	VMOVAPS [dword rdx + 128], ymm14
	VSUBPS ymm11, ymm11, ymm10
	VMOVAPS [dword rdx + 160], ymm12
	VMOVAPS [dword rdx + 192], ymm11
	ADD rdx, 224
	.batch_process_finish:
	ADD rcx, 56
	JZ .return_ok
	.process_single:
	VMOVSS xmm8, [rdi]
	ADD rdi, 4
	VMOVSS xmm9, [rsi]
	ADD rsi, 4
	VSUBSS xmm8, xmm8, xmm9
	VMOVSS [rdx], xmm8
	ADD rdx, 4
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.Nehalem progbits alloc exec nowrite align=16
global _yepCore_Subtract_V64fV64f_V64f_Nehalem
_yepCore_Subtract_V64fV64f_V64f_Nehalem:
%else
section .text
global __yepCore_Subtract_V64fV64f_V64f_Nehalem
__yepCore_Subtract_V64fV64f_V64f_Nehalem:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 7
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 7
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 15
	JZ .source_z_16b_aligned
	.source_z_16b_misaligned:
	MOVSD xmm2, [rdi]
	ADD rdi, 8
	MOVSD xmm1, [rsi]
	ADD rsi, 8
	SUBSD xmm2, xmm1
	MOVSD [rdx], xmm2
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 15
	JNZ .source_z_16b_misaligned
	.source_z_16b_aligned:
	SUB rcx, 14
	JB .batch_process_finish
	.process_batch_prologue:
	MOVUPD xmm2, [rdi]
	MOVUPD xmm8, [byte rdi + 16]
	MOVUPD xmm4, [rsi]
	MOVUPD xmm9, [byte rdi + 32]
	MOVUPD xmm3, [byte rsi + 16]
	MOVUPD xmm12, [byte rdi + 48]
	MOVUPD xmm13, [byte rsi + 32]
	MOVUPD xmm14, [byte rdi + 64]
	MOVUPD xmm10, [byte rsi + 48]
	MOVUPD xmm11, [byte rdi + 80]
	MOVUPD xmm5, [byte rsi + 64]
	SUBPD xmm2, xmm4
	MOVUPD xmm6, [byte rdi + 96]
	MOVUPD xmm7, [byte rsi + 80]
	SUBPD xmm8, xmm3
	MOVAPD [rdx], xmm2
	ADD rdi, 112
	MOVUPD xmm15, [byte rsi + 96]
	SUBPD xmm9, xmm13
	MOVAPD [byte rdx + 16], xmm8
	SUB rcx, 14
	JB .process_batch_epilogue
	align 16
	.process_batch:
	MOVUPD xmm2, [rdi]
	ADD rsi, 112
	SUBPD xmm12, xmm10
	MOVAPD [byte rdx + 32], xmm9
	MOVUPD xmm8, [byte rdi + 16]
	MOVUPD xmm4, [rsi]
	SUBPD xmm14, xmm5
	MOVAPD [byte rdx + 48], xmm12
	MOVUPD xmm9, [byte rdi + 32]
	MOVUPD xmm3, [byte rsi + 16]
	SUBPD xmm11, xmm7
	MOVAPD [byte rdx + 64], xmm14
	MOVUPD xmm12, [byte rdi + 48]
	MOVUPD xmm13, [byte rsi + 32]
	SUBPD xmm6, xmm15
	MOVAPD [byte rdx + 80], xmm11
	MOVUPD xmm14, [byte rdi + 64]
	MOVUPD xmm10, [byte rsi + 48]
	MOVAPD [byte rdx + 96], xmm6
	MOVUPD xmm11, [byte rdi + 80]
	MOVUPD xmm5, [byte rsi + 64]
	SUBPD xmm2, xmm4
	ADD rdx, 112
	MOVUPD xmm6, [byte rdi + 96]
	MOVUPD xmm7, [byte rsi + 80]
	SUBPD xmm8, xmm3
	MOVAPD [rdx], xmm2
	ADD rdi, 112
	MOVUPD xmm15, [byte rsi + 96]
	SUBPD xmm9, xmm13
	MOVAPD [byte rdx + 16], xmm8
	SUB rcx, 14
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 112
	SUBPD xmm12, xmm10
	MOVAPD [byte rdx + 32], xmm9
	SUBPD xmm14, xmm5
	MOVAPD [byte rdx + 48], xmm12
	SUBPD xmm11, xmm7
	MOVAPD [byte rdx + 64], xmm14
	SUBPD xmm6, xmm15
	MOVAPD [byte rdx + 80], xmm11
	MOVAPD [byte rdx + 96], xmm6
	ADD rdx, 112
	.batch_process_finish:
	ADD rcx, 14
	JZ .return_ok
	.process_single:
	MOVSD xmm8, [rdi]
	ADD rdi, 8
	MOVSD xmm9, [rsi]
	ADD rsi, 8
	SUBSD xmm8, xmm9
	MOVSD [rdx], xmm8
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

%ifidn __OUTPUT_FORMAT__, elf64
section .text.SandyBridge progbits alloc exec nowrite align=16
global _yepCore_Subtract_V64fV64f_V64f_SandyBridge
_yepCore_Subtract_V64fV64f_V64f_SandyBridge:
%else
section .text
global __yepCore_Subtract_V64fV64f_V64f_SandyBridge
__yepCore_Subtract_V64fV64f_V64f_SandyBridge:
%endif
	.ENTRY:
	TEST rdi, rdi
	JZ .return_null_pointer
	TEST rdi, 7
	JNZ .return_misaligned_pointer
	TEST rsi, rsi
	JZ .return_null_pointer
	TEST rsi, 7
	JNZ .return_misaligned_pointer
	TEST rdx, rdx
	JZ .return_null_pointer
	TEST rdx, 7
	JNZ .return_misaligned_pointer
	TEST rcx, rcx
	JZ .return_ok
	TEST rdx, 31
	JZ .source_z_32b_aligned
	.source_z_32b_misaligned:
	VMOVSD xmm2, [rdi]
	ADD rdi, 8
	VMOVSD xmm1, [rsi]
	ADD rsi, 8
	VSUBSD xmm2, xmm2, xmm1
	VMOVSD [rdx], xmm2
	ADD rdx, 8
	SUB rcx, 1
	JZ .return_ok
	TEST rdx, 31
	JNZ .source_z_32b_misaligned
	.source_z_32b_aligned:
	SUB rcx, 28
	JB .batch_process_finish
	.process_batch_prologue:
	VMOVUPD ymm2, [rdi]
	VMOVUPD ymm8, [byte rdi + 32]
	VMOVUPD ymm13, [rsi]
	VMOVUPD ymm9, [byte rdi + 64]
	VMOVUPD ymm7, [byte rsi + 32]
	VMOVUPD ymm3, [byte rdi + 96]
	VMOVUPD ymm6, [byte rsi + 64]
	VMOVUPD ymm14, [dword rdi + 128]
	VMOVUPD ymm5, [byte rsi + 96]
	VMOVUPD ymm12, [dword rdi + 160]
	VMOVUPD ymm4, [dword rsi + 128]
	VSUBPD ymm2, ymm2, ymm13
	VMOVUPD ymm11, [dword rdi + 192]
	VMOVUPD ymm15, [dword rsi + 160]
	VSUBPD ymm8, ymm8, ymm7
	VMOVAPD [rdx], ymm2
	ADD rdi, 224
	VMOVUPD ymm10, [dword rsi + 192]
	VSUBPD ymm9, ymm9, ymm6
	VMOVAPD [byte rdx + 32], ymm8
	SUB rcx, 28
	JB .process_batch_epilogue
	align 16
	.process_batch:
	VMOVUPD ymm2, [rdi]
	ADD rsi, 224
	VSUBPD ymm3, ymm3, ymm5
	VMOVAPD [byte rdx + 64], ymm9
	VMOVUPD ymm8, [byte rdi + 32]
	VMOVUPD ymm13, [rsi]
	VSUBPD ymm14, ymm14, ymm4
	VMOVAPD [byte rdx + 96], ymm3
	VMOVUPD ymm9, [byte rdi + 64]
	VMOVUPD ymm7, [byte rsi + 32]
	VSUBPD ymm12, ymm12, ymm15
	VMOVAPD [dword rdx + 128], ymm14
	VMOVUPD ymm3, [byte rdi + 96]
	VMOVUPD ymm6, [byte rsi + 64]
	VSUBPD ymm11, ymm11, ymm10
	VMOVAPD [dword rdx + 160], ymm12
	VMOVUPD ymm14, [dword rdi + 128]
	VMOVUPD ymm5, [byte rsi + 96]
	VMOVAPD [dword rdx + 192], ymm11
	VMOVUPD ymm12, [dword rdi + 160]
	VMOVUPD ymm4, [dword rsi + 128]
	VSUBPD ymm2, ymm2, ymm13
	ADD rdx, 224
	VMOVUPD ymm11, [dword rdi + 192]
	VMOVUPD ymm15, [dword rsi + 160]
	VSUBPD ymm8, ymm8, ymm7
	VMOVAPD [rdx], ymm2
	ADD rdi, 224
	VMOVUPD ymm10, [dword rsi + 192]
	VSUBPD ymm9, ymm9, ymm6
	VMOVAPD [byte rdx + 32], ymm8
	SUB rcx, 28
	JAE .process_batch
	.process_batch_epilogue:
	ADD rsi, 224
	VSUBPD ymm3, ymm3, ymm5
	VMOVAPD [byte rdx + 64], ymm9
	VSUBPD ymm14, ymm14, ymm4
	VMOVAPD [byte rdx + 96], ymm3
	VSUBPD ymm12, ymm12, ymm15
	VMOVAPD [dword rdx + 128], ymm14
	VSUBPD ymm11, ymm11, ymm10
	VMOVAPD [dword rdx + 160], ymm12
	VMOVAPD [dword rdx + 192], ymm11
	ADD rdx, 224
	.batch_process_finish:
	ADD rcx, 28
	JZ .return_ok
	.process_single:
	VMOVSD xmm8, [rdi]
	ADD rdi, 8
	VMOVSD xmm9, [rsi]
	ADD rsi, 8
	VSUBSD xmm8, xmm8, xmm9
	VMOVSD [rdx], xmm8
	ADD rdx, 8
	SUB rcx, 1
	JNZ .process_single
	.return_ok:
	XOR eax, eax
	.return:
	VZEROUPPER
	RET
	.return_null_pointer:
	MOV eax, 1
	JMP .return
	.return_misaligned_pointer:
	MOV eax, 2
	JMP .return

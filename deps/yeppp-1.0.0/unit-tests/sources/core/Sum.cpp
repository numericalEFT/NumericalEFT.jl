...
static Yep32s Test_Sum_V32f_S32f(Yep64u supportedIsaFeatures, Yep64u supportedSimdFeatures, Yep64u supportedSystemFeatures) {
	...
	const DescriptorPointer defaultDescriptor = findDefaultDescriptor(_dispatchTable_yepCore_Sum_V32f_S32f);
	...
	status = yepRandom_WELL1024a_GenerateUniform_S32fS32f_V32f_Acc32(&rng, 0.0f, 1000.0f, vArray, YEP_COUNT_OF(vArray));
	assert(status == YepStatusOk);
	status = yepRandom_WELL1024a_GenerateUniform_S32fS32f_V32f_Acc32(&rng, -1.0f, 1.0f, &sumInit, 1);
	assert(status == YepStatusOk);

	for (DescriptorPointer descriptor = &_dispatchTable_yepCore_Sum_V32f_S32f[0]; descriptor != defaultDescriptor; descriptor++) {
		...
		if (unsupportedRequiredFeatures == 0) {
			for (YepSize vOffset = 0; vOffset < 64 / sizeof(Yep32f); vOffset++) {
				for (YepSize length = 0; length < 64; length += 1) {
					sumRef = sumInit;
					status = defaultImplementation(&vArray[vOffset], &sumRef, length);
					assert(status == YepStatusOk);

					sum = sumInit;
					status = descriptor->function(&vArray[vOffset], &sum, length);
					assert(status == YepStatusOk);

					const Yep32f ulpError = yepBuiltin_Abs_32f_32f(sumRef - sum) / yepBuiltin_Ulp_32f_32f(sumRef);
					if (ulpError > 1000.0f) {
						failedTests += 1;
						reportFailedTest("yepCore_Sum_V32f_S32f", descriptor->microarchitecture, float(ulpError));
			...
			reportPassedTest("yepCore_Sum_V32f_S32f", descriptor->microarchitecture);
		} else {
			reportSkippedTest("yepCore_Sum_V32f_S32f", descriptor->microarchitecture);
		}
next_descriptor:
		continue;
	}
...
static Yep32s Test_Sum_V64f_S64f(Yep64u supportedIsaFeatures, Yep64u supportedSimdFeatures, Yep64u supportedSystemFeatures) {
	YepRandom_WELL1024a rng;
	YepStatus status = yepRandom_WELL1024a_Init(&rng);
	assert(status == YepStatusOk);
	...
	typedef const FunctionDescriptor<FunctionPointer>* DescriptorPointer;
	const DescriptorPointer defaultDescriptor = findDefaultDescriptor(_dispatchTable_yepCore_Sum_V64f_S64f);
	const FunctionPointer defaultImplementation = defaultDescriptor->function;
	Yep32s failedTests = 0;
	...
	status = yepRandom_WELL1024a_GenerateUniform_S64fS64f_V64f_Acc64(&rng, 0.0, 1000.0, vArray, YEP_COUNT_OF(vArray));
	assert(status == YepStatusOk);
	status = yepRandom_WELL1024a_GenerateUniform_S64fS64f_V64f_Acc64(&rng, -1.0, 1.0, &sumInit, 1);
	assert(status == YepStatusOk);

	for (DescriptorPointer descriptor = &_dispatchTable_yepCore_Sum_V64f_S64f[0]; descriptor != defaultDescriptor; descriptor++) {
		const Yep64u unsupportedRequiredFeatures = (descriptor->isaFeatures & ~supportedIsaFeatures) |
			(descriptor->simdFeatures & ~supportedSimdFeatures) |
			(descriptor->systemFeatures & ~supportedSystemFeatures);
		if (unsupportedRequiredFeatures == 0) {
			for (YepSize vOffset = 0; vOffset < 64 / sizeof(Yep64f); vOffset++) {
				for (YepSize length = 0; length < 64; length += 1) {
					...
					const Yep64f ulpError = yepBuiltin_Abs_64f_64f(sumRef - sum) / yepBuiltin_Ulp_64f_64f(sumRef);
					if (ulpError > 1000.0f) {
						failedTests += 1;
						reportFailedTest("yepCore_Sum_V64f_S64f", descriptor->microarchitecture, float(ulpError));
						goto next_descriptor;
					}
				}
				for (YepSize length = 1024; length < 1088; length += 1) {
			...
			reportPassedTest("yepCore_Sum_V64f_S64f", descriptor->microarchitecture);
		} else {
			reportSkippedTest("yepCore_Sum_V64f_S64f", descriptor->microarchitecture);
		}
next_descriptor:
		continue;
	}
...
int main(int argc, char** argv) {
	YepBoolean testSum_V32f_S32f = YepBooleanFalse;
	YepBoolean testSum_V64f_S64f = YepBooleanFalse;
	if (argc == 1) {
		/* No tests specified: run all tests*/
		testSum_V32f_S32f = YepBooleanTrue;
		testSum_V64f_S64f = YepBooleanTrue;
	} else {
	...
	Yep32s failedTests = 0;
	if YEP_LIKELY(testSum_V32f_S32f)
		failedTests += Test_Sum_V32f_S32f(supportedIsaFeatures, supportedSimdFeatures, supportedSystemFeatures);
	if YEP_LIKELY(testSum_V64f_S64f)
		failedTests += Test_Sum_V64f_S64f(supportedIsaFeatures, supportedSimdFeatures, supportedSystemFeatures);
	return failedTests;
}

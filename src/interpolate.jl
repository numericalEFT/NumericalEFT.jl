...
- y: y
"""
@inline function linear2D(xgrid, ygrid, data, x, y)
    if (x <= xgrid.grid[firstindex(xgrid)] || x >= xgrid.grid[lastindex(xgrid)] || y <= ygrid.grid[firstindex(ygrid)] || y >= grid.grid[lastindex(ygrid)])
        return 0.0
    end
    xidx0 = xgrid.floor(x)
    yidx0 = ygrid.floor(y)
    ...
    dy0 = y - ygrid.grid[yidy0]
    dy1 = ygrid.grid[yidy0 + 1] - y
